...
public class MdmsCacheServiceTest {
    ...
    @Test
    public void testUpdateCache_WithDbVerification_Success() {
        String tenantId = "pb";
        String schemaCode = "ws-services-masters.billingPeriod";
        String uniqueIdentifier = "Metered";
        String id = "billing-1";

        ReflectionTestUtils.setField(mdmsCacheService, "dbLoadEnabled", true);

        Map<String, Object> dbData = new LinkedHashMap<>();
        dbData.put("code", "Metered");
        dbData.put("connectionType", "Metered");
        dbData.put("billingCycle", "monthly");

        Map<String, Object> dbRow = new HashMap<>();
        dbRow.put("tenantid", tenantId);
        dbRow.put("schemacode", schemaCode);
        dbRow.put("uniqueidentifier", uniqueIdentifier);
        dbRow.put("id", id);
        dbRow.put("isactive", true);
        dbRow.put("data", dbData);

        when(mdmsDataRepository.search(tenantId, schemaCode, uniqueIdentifier, id))
                .thenReturn(Arrays.asList(dbRow));

        Map<String, Object> mdmsObj = new HashMap<>();
        mdmsObj.put("id", id);
        mdmsObj.put("tenantId", tenantId);
        mdmsObj.put("schemaCode", schemaCode);
        mdmsObj.put("uniqueIdentifier", uniqueIdentifier);
        ...
        JSONArray masterData = MDMSApplicationRunnerImpl.getTenantMap().get(tenantId).get("ws-services-masters").get("billingPeriod");
        assertNotNull(masterData);
        assertEquals(1, masterData.size());
        Map<?, ?> record = (Map<?, ?>) masterData.get(0);
        assertEquals("monthly", record.get("billingCycle"));
    }

    @Test
    public void testUpdateCache_WithDbVerification_NotFoundInDb_SkipsCacheUpdate() {
        String tenantId = "pb";
        String schemaCode = "ws-services-masters.billingPeriod";
        String uniqueIdentifier = "UnpersistedRecord";
        String id = "unpersisted-id-1";
        ...
        recordData.put("code", "UnpersistedRecord");
        ...
        Map<String, Map<String, Map<String, JSONArray>>> tenantMap = MDMSApplicationRunnerImpl.getTenantMap();
        if (tenantMap.containsKey(tenantId) && tenantMap.get(tenantId).containsKey("ws-services-masters")) {
            JSONArray masterData = tenantMap.get(tenantId).get("ws-services-masters").get("billingPeriod");
            if (masterData != null) {
