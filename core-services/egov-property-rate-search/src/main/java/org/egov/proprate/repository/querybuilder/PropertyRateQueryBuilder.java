...
public class PropertyRateQueryBuilder {
    ...
    // UPDATED: Added 'revenue_' prefix to all master tables
    private static final String BASE_FROM =
            ...
            "INNER JOIN revenue_sub_category_master sc ON prm.sub_category_id = sc.sub_category_id " +
            "INNER JOIN revenue_usage_category_master uc ON sc.usage_category_id = uc.usage_category_id ";
    ...
    // UPDATED: revenue_district_master
    ...
    // UPDATED: revenue_village_master
    ...
    // UPDATED: revenue_segment_master
    ...
    // UPDATED: revenue_usage_category_master
    ...
    // UPDATED: revenue_sub_category_master
    private static final String FETCH_SUB_CATEGORIES =
            "SELECT sc.sub_category_name, sc.sub_category_id FROM revenue_sub_category_master sc " +
            "WHERE sc.usage_category_id = :usageCategoryId ORDER BY sc.sub_category_name";
 ...
 private static final String MAPPED_PROPERTIES =
    ...
    public String getSearchQuery(RateSearchRequest request, Map<String, Object> params) {
        ...
        boolean isRateCheck =
                criteria.getIsRateCheck() != null && criteria.getIsRateCheck();

        /* =========================
           SCENARIO 1 : RATE CHECK
           ========================= */

        if (isRateCheck) {

            StringBuilder query = new StringBuilder();
            query.append(BASE_SELECT)
                 .append(BASE_FROM)
                 .append(BASE_WHERE);
        ...
        if (!ObjectUtils.isEmpty(criteria.getUsageCategoryId())) {
            params.put("usageCategoryId",
                    Integer.parseInt(criteria.getUsageCategoryId()));
    ...
    private void addFilters(StringBuilder query,
        ...
        if (!ObjectUtils.isEmpty(criteria.getSegmentId())) {
            ...
            params.put("segmentId",
                    Integer.parseInt(criteria.getSegmentId()));
        }
        ...
        if (!ObjectUtils.isEmpty(criteria.getSubCategoryId())) {
            query.append(" AND sc.sub_category_id = :subCategoryId");
            params.put("subCategoryId",
                    Integer.parseInt(criteria.getSubCategoryId()));
        }
