...
import org.egov.wf.repository.rowmapper.EmployeeRowMapper;
import org.egov.wf.repository.rowmapper.FileEmployeeRowMapper;
import org.egov.wf.repository.rowmapper.WorkflowRowMapper;
import org.egov.wf.repository.rowmapper.WorkflowStatusRowMapper;
...
public class WorKflowRepository {
	...
	private WorkflowStatusRowMapper workflowStatusRowMapper;
	@Autowired
	EmployeeRowMapper employeeRowMapper;

	@Autowired
	FileEmployeeRowMapper fileemployeeRowMapper;

	@Autowired
	public WorKflowRepository(WorkflowQueryBuilder queryBuilder, JdbcTemplate jdbcTemplate, WorkflowRowMapper rowMapper,
			WorkflowStatusRowMapper workflowStatusRowMapper) {
		this.queryBuilder = queryBuilder;
		this.jdbcTemplate = jdbcTemplate;
		this.rowMapper = rowMapper;
		this.workflowStatusRowMapper = workflowStatusRowMapper;
	}
	 ...
	 * @return The parsed response from the search query
	 */
	public List<ProcessInstance> getProcessInstancesv2(ProcessInstanceSearchCriteria criteria) {
		List<Object> preparedStmtList = new ArrayList<>();
		...
		String query = queryBuilder.getProcessInstanceSearchQueryByIdv2(ids, preparedStmtList);
		log.debug("query for status search: " + query + " params: " + preparedStmtList);
	 ...
	 * @return The parsed response from the search query
	 */
	public List<ProcessInstance> getProcessInstancesV3(ProcessInstanceSearchCriteria criteria) {
		List<Object> preparedStmtList = new ArrayList<>();

		List<String> ids = getProcessInstanceIdsV3(criteria);
	 ...
	 * @return
	 */
	public List<Employe> getEmployeeByApplication(List<String> criteria, String tenantId) {

		List<Object> preparedStmtList = new ArrayList<>();

		String query = queryBuilder.getEmployeeByApplication(criteria, tenantId, preparedStmtList);
		log.debug("query for status search: " + query + " params: " + preparedStmtList);
		return jdbcTemplate.query(query, preparedStmtList.toArray(), employeeRowMapper);
	 ...
	 * @return
	 */
	public List<FileEmployees> getEmployeeByApplicationfile(List<String> criteria, String tenantId) {

		List<Object> preparedStmtList = new ArrayList<>();

		String query = queryBuilder.getFileEmployeeByApplication(criteria, tenantId, preparedStmtList);
		log.debug("query for status search: " + query + " params: " + preparedStmtList);
		return jdbcTemplate.query(query, preparedStmtList.toArray(), fileemployeeRowMapper);
	...
	public List getProcessInstancesStatusCountv2(ProcessInstanceSearchCriteria criteria) {
		List<Object> preparedStmtList = new ArrayList<>();
		String query = queryBuilder.getProcessInstanceCountv2(criteria, preparedStmtList, Boolean.TRUE);
		return jdbcTemplate.queryForList(query, preparedStmtList.toArray());
	}
	...
	private List<String> getProcessInstanceIdsV3(ProcessInstanceSearchCriteria criteria) {
		List<Object> preparedStmtList = new ArrayList<>();
		String query = queryBuilder.getProcessInstanceIdsV3(criteria, preparedStmtList);
		log.info(query);
		log.info(preparedStmtList.toString());
		return jdbcTemplate.query(query, preparedStmtList.toArray(), new SingleColumnRowMapper<>(String.class));
	}

	public List<String> fetchEscalatedApplicationsBusinessIdsFromDb(RequestInfo requestInfo,
		...
		/*
		 * 
		 * String query = queryBuilder.getInboxApplicationsBusinessIdsQuery(criteria,
		 * preparedStmtList); List<String> inboxApplicationsBusinessIds =
		 * jdbcTemplate.query(query, preparedStmtList.toArray(), new
		 * SingleColumnRowMapper<>(String.class));
		 * log.info(inboxApplicationsBusinessIds.toString()); preparedStmtList.clear();
		 * 
		 * // (DONE) 2nd step is to fetch businessIds of inbox applications which have
		 * been autoEscalated at least once in their wf // (DONE) For this step, fetch
		 * AUTO_ESCALATION_EMPLOYEES uuids based on role codes by doing a call to user
		 * service // (PENDING) Also, add the call to mdms service for filtering out
		 * states which need to be excluded
		 * 
		...
		log.info(query);
		List<String> escalatedApplicationsBusinessIds = jdbcTemplate.query(query, preparedStmtList.toArray(),
				new SingleColumnRowMapper<>(String.class));
		preparedStmtList.clear();
		log.info(escalatedApplicationsBusinessIds.toString());
		// 3rd step is to do a simple search on these business ids(DONE IN WORKFLOW
		// SERVICE)
	...
	public Integer getEscalatedApplicationsCount(RequestInfo requestInfo, ProcessInstanceSearchCriteria criteria) {
		List<Object> preparedStmtList = new ArrayList<>();
		String query = queryBuilder.getEscalatedApplicationsCount(requestInfo, criteria,
				(ArrayList<Object>) preparedStmtList);
		Integer count = jdbcTemplate.queryForObject(query, preparedStmtList.toArray(), Integer.class);
		return count;
	}

	public List<ProcessInstance> getStatusSearch(ProcessInstanceSearchCriteria criteria) {
		List<Object> preparedStmtList = new ArrayList<>();
		String query = queryBuilder.getStatusSearch(criteria, preparedStmtList);

		log.debug("query for status search: " + query + " params: " + preparedStmtList);
		return jdbcTemplate.query(query, preparedStmtList.toArray(), workflowStatusRowMapper);
	}
}