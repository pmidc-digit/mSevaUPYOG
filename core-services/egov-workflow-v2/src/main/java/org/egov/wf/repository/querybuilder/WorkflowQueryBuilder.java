...
public class WorkflowQueryBuilder {
	...
	private static final String QUERY_APP = "select emp.uuid as id , emp.name from public.eg_hrms_employee emp,public.eg_wf_assignee_v2 ass,public.eg_wf_processinstance_v2 prc\r\n"
			+ "where prc.id= ass.processinstanceid " + "and emp.uuid=ass.assignee and prc.tenantid=?";
	private static final String QUERY_APP_file = "select emp.uuid as id , emp.name,ass.is_current,ass.createdtime,ass.lastmodifiedtime from public.eg_hrms_employee emp,\r\n"
			+ "public.eg_wf_assignee_v2 ass,public.eg_wf_processinstance_v2 prc where prc.id= ass.processinstanceid\r\n"
			+ "and emp.uuid=ass.assignee and prc.tenantid=?";
	...
	private static final String QUERYV2 = " SELECT pi.*,st.*,ac.*,doc.*,pi.id as wf_id,pi.lastModifiedTime as wf_lastModifiedTime,pi.createdTime as wf_createdTime,"
			+ "       pi.createdBy as wf_createdBy,pi.lastModifiedBy as wf_lastModifiedBy,pi.status as pi_status, pi.tenantid as pi_tenantid, "
			+ "       doc.lastModifiedTime as doc_lastModifiedTime,doc.createdTime as doc_createdTime,doc.createdBy as doc_createdBy,"
			+ "       doc.lastModifiedBy as doc_lastModifiedBy,doc.tenantid as doc_tenantid,doc.id as doc_id,asg.assignee as assigneeuuid,"
			+ "       st.uuid as st_uuid,st.tenantId as st_tenantId, ac.uuid as ac_uuid,ac.tenantId as ac_tenantId,ac.action as ac_action"
	...
	private static final String BASE_CLAUSE = " select id from eg_wf_processinstance_v2 pi_outer  ,eg_wf_assignee_v2 asg_inner\r\n"
			+ " WHERE ";
	private static final String SEARCH_CLAUSE = "select * from eg_wf_processinstance_v2 pi_outer ,eg_wf_assignee_v2 asg_inner,\r\n"
			+ "	public.eg_wf_state_v2 stat WHERE pi_outer.status=stat.uuid and  asg_inner.processinstanceid=pi_outer.id and asg_inner.is_current='true'";
	...
	private String getProcessInstanceSearchQueryWithoutPagination(ProcessInstanceSearchCriteria criteria,
		...
		if (!CollectionUtils.isEmpty(tenantSpecificStatuses)) {
			builder.append(" and CONCAT (pi.tenantid,':',pi.status)  IN (").append(createQuery(tenantSpecificStatuses))
					.append(")");
		...
		builder.append(" AND asg.is_current='true'");
		return builder.toString();

	}

	public String getProcessInstanceIdsV3(ProcessInstanceSearchCriteria criteria, List<Object> preparedStmtList) {
		...
		if (criteria.getAssignee() != null) {
			with_query_builder.append(" and " + "asg_inner.assignee = ? ");
		...
		if (!CollectionUtils.isEmpty(businessIds)) {
			if (isFuzzyEnabled) {
				with_query_builder.append(" and pi_outer.businessId LIKE ANY(ARRAY[ ").append(createQuery(businessIds))
						.append("])");
				addToPreparedStatementForFuzzySearch(preparedStmtList, businessIds);
			} else {
				with_query_builder.append(" and pi_outer.businessId IN ( ").append(createQuery(businessIds))
						.append(")");
				addToPreparedStatement(preparedStmtList, businessIds);
			}
		}
	...
	public String getProcessInstanceSearchQueryByIdv2(List<String> ids, List<Object> preparedStmtList) {

		StringBuilder builder = new StringBuilder(QUERYV2);
	...
	public String getInboxIdQuery(ProcessInstanceSearchCriteria criteria, List<Object> preparedStmtList,
		...
		String with_query = BASE_CLAUSE + "asg_inner.processinstanceid=pi_outer.id and asg_inner.is_current='true'";
		/*
		 * String with_query_status = WITH_CLAUSE + " pi_outer.lastmodifiedTime = (" +
		...
		// StringBuilder with_query_builder_status = new
		// StringBuilder(with_query_status);
		if (criteria.getIsAssignedToMeCount() != null && criteria.getIsAssignedToMeCount()) {
			with_query_builder.append(" and " + "asg_inner.assignee = ? and pi_outer.tenantid = ? ");
			preparedStmtList.add(criteria.getAssignee());
			preparedStmtList.add(criteria.getTenantId());
		} else if (!config.getAssignedOnly() && !CollectionUtils.isEmpty(tenantSpecificStatus)) {
			String clause = " and ("
					+ "            		asg_inner.assignee = ? and pi_outer.tenantid = ? ) {{OR_CLUASE_PLACEHOLDER}} ";
			...
			if (!isNull(criteria.getModuleName()) && criteria.getModuleName().equals("BPAREG")) {
				...
				if (CollectionUtils.isEmpty(tenantSpecificStatus)
						&& !CollectionUtils.isEmpty(statusesIrrespectiveOfTenant)) {
					...
					StringBuilder statusQuery = new StringBuilder(" OR pi_outer.status IN (")
							.append(createQuery(statusesIrrespectiveOfTenant)).append(")");
	...
	private String getStatusRelatedWhereClause(List<String> statuses, List<String> tenantSpecificStatus,
			List<Object> preparedStmtList) {
	...
	private String getTenantSpecificStatusClause(List<String> tenantSpecificStatus) {
		StringBuilder builder = new StringBuilder(" OR (pi_outer.tenantid || ':' || pi_outer.status) IN (")
				.append(createQuery(tenantSpecificStatus)).append(")");
		return builder.toString();
	}
	 ...
	 * @return Query with pagination
	 */
	private void addPagination(StringBuilder query, List<Object> preparedStmtList,
			ProcessInstanceSearchCriteria criteria) {
		int limit = config.getDefaultLimit();
		int offset = config.getDefaultOffset();
		query.append(" OFFSET ? ");
	...
	public String getProcessInstanceCountv2(ProcessInstanceSearchCriteria criteria, List<Object> preparedStmtList,
			boolean statuCount) {
		String finalQuery = getProcessInstanceSearchQueryWithoutPagination(criteria, preparedStmtList);
		String countQuery = null;
		if (statuCount) {
	...
	public String getInboxApplicationsBusinessIdsQuery(ProcessInstanceSearchCriteria criteria,
			ArrayList<Object> preparedStmtList) {
	...
	public String getEscalatedApplicationsCount(RequestInfo requestInfo, ProcessInstanceSearchCriteria criteria,
			ArrayList<Object> preparedStmtList) {
	...
	public String getAutoEscalatedApplicationsFinalQuery(RequestInfo requestInfo,
			ProcessInstanceSearchCriteria criteria, ArrayList<Object> preparedStmtList) {
		String autoEscalatedApplicationsRankedQuery = getAutoEscalatedApplicationsRankedQuery(criteria,
				preparedStmtList);
	...
	public String getAutoEscalatedApplicationsRankedQuery(ProcessInstanceSearchCriteria criteria,
			ArrayList<Object> preparedStmtList) {
		String autoEscalatedApplicationsBusinessIdsQuery = getAutoEscalatedApplicationsBusinessIdsQuery(criteria,
				preparedStmtList);
		String query = RANK_WRAPPER.replace("{BASE_QUERY}", autoEscalatedApplicationsBusinessIdsQuery);
		return query;
	}

	public String getAutoEscalatedApplicationsBusinessIdsQuery(ProcessInstanceSearchCriteria criteria,
			ArrayList<Object> preparedStmtList) {
...
//        }
	...
	public String getEmployeeByApplication(List<String> criteria, String tenantId, List<Object> preparedStmtList) {
		String with_query = QUERY_APP;
		StringBuilder builderQuery = new StringBuilder(with_query);
		preparedStmtList.add(tenantId);
		List<String> businessIds = criteria;
		if (!CollectionUtils.isEmpty(businessIds)) {
			builderQuery.append(" and prc.businessid IN (").append(createQuery(businessIds)).append(")");
			addToPreparedStatement(preparedStmtList, businessIds);
		}
		return builderQuery.toString();
	}

	public String getFileEmployeeByApplication(List<String> criteria, String tenantId, List<Object> preparedStmtList) {
		String with_query = QUERY_APP_file;
		StringBuilder builderQuery = new StringBuilder(with_query);
		preparedStmtList.add(tenantId);
		List<String> businessIds = criteria;
		if (!CollectionUtils.isEmpty(businessIds)) {
	...
	public String getStatusSearch(ProcessInstanceSearchCriteria criteria, List<Object> preparedStmtList) {
		String with_query = SEARCH_CLAUSE;
		StringBuilder builderQuery = new StringBuilder(with_query);
		List<String> businessIds = criteria.getBusinessIds();
		if (!CollectionUtils.isEmpty(businessIds)) {
			builderQuery.append(" and pi_outer.businessid IN (").append(createQuery(businessIds)).append(")");
			addToPreparedStatement(preparedStmtList, businessIds);
		}
		if (!criteria.getAssignee().isEmpty()) {
			// addClauseIfRequired(with_query, preparedStmtList);
			builderQuery.append(" and asg_inner.assignee = ? ");
			preparedStmtList.add(criteria.getAssignee());
		}
