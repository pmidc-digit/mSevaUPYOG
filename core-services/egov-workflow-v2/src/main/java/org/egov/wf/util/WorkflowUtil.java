...
public class WorkflowUtil {
	...
	/*
	 * public List<String> getActionableStatusesForRole(RequestInfo requestInfo,
	 * List<BusinessService> businessServices,ProcessInstanceSearchCriteria
	 * criteria){
	 * 
	 * String tenantId; List<String> userRoleCodes; Map<String,List<String>>
	 * tenantIdToUserRolesMap = getTenantIdToUserRolesMap(requestInfo);
	 * Map<String,List<BusinessService>> tenantIdToBuisnessSevicesMap =
	 * getTenantIdToBuisnessSevicesMap(businessServices); Map<String,Set<String>>
	 * stateToRoleMap = getStateToRoleMap(businessServices); List<String>
	 * actionableStatuses = new LinkedList<>();
	 * 
	 * for(Map.Entry<String,List<String>> entry :
	 * tenantIdToUserRolesMap.entrySet()){
	 * if(entry.getKey().equals(criteria.getTenantId())){ List<BusinessService>
	 * businessServicesByTenantId ; if(config.getIsStateLevel()){
	 * businessServicesByTenantId =
	 * tenantIdToBuisnessSevicesMap.get(entry.getKey().split("\\.")[0]); }else{
	 * businessServicesByTenantId =
	 * tenantIdToBuisnessSevicesMap.get(entry.getKey()); }
	 * businessServicesByTenantId.forEach(service -> { List<State> states =
	 * service.getStates(); states.forEach(state -> { Set<String> stateRoles =
	 * stateToRoleMap.get(state.getUuid()); if(!CollectionUtils.isEmpty(stateRoles)
	...
	/**
	 * Have to find statuses on which the user can take action There are 4 possible
	 * scenarios that has to be covered: 1. user has tenantLevel Role and the config
	 * is also tenantLevel 2. user has tenantLevel Role and the config is stateLevel
	 * 3. user has stateLevel Role and the config is tenantLevel 4. user has
	 * stateLevel Role and the config is also stateLevel
	 *
	 * roleTenantAndStatusMapping map captures the following sample data structure:
	 *
	 * ROLE TENANTID STATUSES TL_CEMP pb.amritsar UUID1,UUID2,.... TL_CEMP
	 * pb.jalandhar UUID3,UUID4,.... PT_CEMP pb UUID5,UUID6,....
	 *
	 * @param requestInfo
	 * @param criteria
	 * @return
	...
	public void enrichStatusesInSearchCriteria(RequestInfo requestInfo, ProcessInstanceSearchCriteria criteria) {
		...
		Map<String, Map<String, List<String>>> roleTenantAndStatusMapping = businessServiceRepository
				.getRoleTenantAndStatusMapping(requestInfo);
		...
		for (Map.Entry<String, List<String>> entry : roleToTenantIdMap.entrySet()) {
			...
			/**
			 * role: Role of the user tenantIds: Tenants for which he has the above
			 * particular role
			 */
			...
			for (Map.Entry<String, List<String>> tenantEntry : tenantToStatuses.entrySet()) {
				...
				/**
				 * Handles Use Case 1: Example: role is TL_CEMP for tenantId pb.amritsar and
				 * config is tenant level. If the tenaantKey is equal to pb.amritsar we have to
				 * search all applications with status in statuses and tenantId = pb.amritsar.
				 * Since we do bulk search (we can't have multiple IN clause) we use derived
				 * column to search which is tenanat:statusUUID
				 */
				if (!isStatelevelRolePresent && tenantIds.contains(tenantKey))
					tenantSpecificStatuses
							.addAll(statuses.stream().map(s -> tenantKey + ":" + s).collect(Collectors.toList()));

				/**
				 * Handles Use Case 2: User has TL_CEMP role for pb.amritsar and pb.jalandhar
				 * and the config is statelevel. In this case we have to search all applications
				 * having tenantId either pb.amritsar or pb.jalandhar and status in the list
				 * statuses
				 *
				 */
				if (!isStatelevelRolePresent && tenantKey.equalsIgnoreCase(config.getStateLevelTenantId())) {
					for (String tenantId : tenantIds) {
						tenantSpecificStatuses
								.addAll(statuses.stream().map(s -> tenantId + ":" + s).collect(Collectors.toList()));
					}
				}

				/**
				 * Handles Use Case 3 and 4: If the user has state level role he can take action
				 * all applications with status in statuses irrespective of the tenantId of the
				 * application
				 */
...
//    }
	 ...
	 * 
	 * @param processStateAndActions List of ProcessStateAndAction whose businessIds
	 *                               are to be fetched
	 ...
	 * 
	 * @param businessId             The businessId whose latest record has to be
	 *                               fetched
	 ...
	 */
	public ProcessStateAndAction getLatestProcessStateAndAction(String businessId,
		...
		for (ProcessStateAndAction processStateAndAction : processStateAndActions) {
			if (processStateAndAction.getProcessInstanceFromRequest().getBusinessId().equalsIgnoreCase(businessId)
					&& maxTime < processStateAndAction.getProcessInstanceFromRequest().getAuditDetails()
							.getLastModifiedTime()) {
	...
	private Boolean isTenantIdValid(String roleTenantId, String applicationTeanantId) {
		...
		// If the role tenantId is statelevel it can take action
			...
			isTenantIdValid = true;
		else if (applicationTeanantId.equalsIgnoreCase(roleTenantId.split("\\.")[0]))
			isTenantIdValid = true;
