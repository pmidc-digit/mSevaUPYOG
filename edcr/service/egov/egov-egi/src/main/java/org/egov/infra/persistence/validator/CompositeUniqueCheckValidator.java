...
public class CompositeUniqueCheckValidator implements ConstraintValidator<CompositeUnique, Object> {
    ...
    private boolean checkCompositeUniqueKey(final Object arg0, final Number id) throws IllegalAccessException {
        ...
        final CriteriaQuery<Object> criteria = entityManager.getCriteriaBuilder().createQuery(Object.class);
        ...
        for (final String fieldName : unique.fields()) {
            ...
            if (unique.checkForNull() && fieldValue == null)
                predicates.add(entityManager.getCriteriaBuilder().isNull(root.get(fieldName)));
            else if (fieldValue instanceof String)
                predicates.add(entityManager.getCriteriaBuilder().equal(
                        entityManager.getCriteriaBuilder().lower(root.get(fieldName)), ((String) fieldValue).toLowerCase()));
            else
                predicates.add(entityManager.getCriteriaBuilder().equal(root.get(fieldName), fieldValue));
        }
        if (id != null)
            predicates.add(entityManager.getCriteriaBuilder().notEqual(root.get(unique.id()), id));
        criteria.select(root.get(unique.id())).where(predicates.toArray(new Predicate[0]));
