...
public class UniqueDateOverlapValidator implements ConstraintValidator<UniqueDateOverlap, Object> {
    ...
    private boolean checkUnique(Object object) throws IllegalAccessException {
        ...
        Predicate checkFromDate = builder.and(
                builder.lessThanOrEqualTo(root.<Date>get(uniqueDateOverlap.fromField()), fromDate),
                builder.greaterThanOrEqualTo(root.<Date>get(uniqueDateOverlap.toField()), fromDate));
        Predicate checkToDate = builder.and(
                builder.lessThanOrEqualTo(root.<Date>get(uniqueDateOverlap.fromField()), toDate),
                builder.greaterThanOrEqualTo(root.<Date>get(uniqueDateOverlap.toField()), toDate));
        Predicate checkFromAndToDate = builder.and(
                builder.greaterThanOrEqualTo(root.<Date>get(uniqueDateOverlap.fromField()), fromDate),
                builder.lessThanOrEqualTo(root.<Date>get(uniqueDateOverlap.toField()), toDate));
        uniqueCheck = builder.and(uniqueCheck, builder.or(checkFromDate, checkToDate, checkFromAndToDate));
        if (id != null)
            uniqueCheck = builder.and(uniqueCheck, builder.notEqual(root.get(uniqueDateOverlap.id()), id));
        query.select(builder.literal(1)).where(uniqueCheck);
