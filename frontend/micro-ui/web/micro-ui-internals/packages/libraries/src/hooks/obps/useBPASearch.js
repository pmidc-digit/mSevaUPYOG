...
//     sla: application?.status.match(/^(APPROVED)$/)
//       ? "CS_NA"
//       : convertMillisecondsToDays(workflowInstances[application?.applicationNo].businesssServiceSla),
...
const combineResponse = (applications, workflowData) => {
  ...
  return applications.map((application) => {
    ...
    return {
      ...
      sla: application?.status?.match(/^(APPROVED)$/)
        ? "CS_NA"
        : wf?.businessServiceSla // <- corrected spelling
        ? convertMillisecondsToDays(wf.businessServiceSla)
        : null,
...
const useBPASearch = (tenantId, filters = {}, config = {}) => {
  ...
  return {
      ...
      async () => {
        ...
        for (var key in tenantMap) {
          for (let i = 0; i < appNumbers.length / 100; i++) {
            try {
              let payload = await Digit.WorkflowService.getAllApplication(key, {
                businessIds: tenantMap[key]?.slice(i * 100, i * 100 + 100)?.join(),
              });
    ...
    revalidate: () => client.removeQueries(["BPA_SEARCH", tenantId, filters]),
  };
};
