...
const usePGRInbox = ({ tenantId, filters = {}, config }) => {
  const { pgrQuery = {}, pgrfilters = {}, wfFilters = {}, sortBy, sortOrder, limit, offset } = filters;
  ...
  const tenants = filters?.filters?.pgrfilters?.tenants;
  const status = filters?.filters?.pgrfilters?.applicationStatus;
