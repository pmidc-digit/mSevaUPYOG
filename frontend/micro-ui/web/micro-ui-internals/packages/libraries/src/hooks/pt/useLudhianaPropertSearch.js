...
const useLudhianaPropertSearch = ({  filters, searchedFrom="" }, config = {}) => {
...
//   };
  ...
  const { isLoading, error, data, isSuccess } = useQuery(["propertySearchList", filters, config], () => Digit.PTService.ludhianaSearch({filters}), {    
    ...config,
  });

  return { isLoading, error, data, isSuccess, revalidate: () => client.invalidateQueries(["propertySearchList", filters]) };
};

export default useLudhianaPropertSearch;