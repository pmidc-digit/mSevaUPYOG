...
const usePetDocumentSearch = ({ petdetail }, config = {}) => {
  ...
  const applicationNumber = petdetail?.applicationNumber;

  const filesArray = petdetail?.documents?.map((value) => value?.filestoreId);
  const { isLoading, error, data } = useQuery([`ptDocuments-${applicationNumber}`, filesArray], () =>
  ...
  return {
    ...
    revalidate: () => client.invalidateQueries([`ptDocuments-${applicationNumber}`, filesArray]),
  };
};
