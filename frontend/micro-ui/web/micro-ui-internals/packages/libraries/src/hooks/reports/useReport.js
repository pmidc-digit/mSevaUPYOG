...
const useReportMeta = {
  ...
  fetchReportData: (moduleName, reportName, tenantId, searchParams, config = {}) =>
    useQuery(
      ["reportMetaData", moduleName, searchParams],
      () => ReportsService.fetchReportsData({ moduleName, reportName, tenantId, searchParams }),
      config
    ),

  fetchReportData1: (moduleName, reportName, tenantId, searchParams, config = {}) =>
    useQuery(
      ["reportMetaData", moduleName, searchParams],
      () => ReportsService.fetchReportsData1({ moduleName, reportName, tenantId, searchParams }),
      config
    ),
};
