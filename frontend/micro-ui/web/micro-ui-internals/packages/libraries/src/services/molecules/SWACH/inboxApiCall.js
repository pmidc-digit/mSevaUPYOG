...
/**
 * Builds the inbox filter object for the API call
 */
const buildInboxFilters = ({ tenantId, filters = {} }) => {
  ...
  const status = nestedFilters?.swachfilters?.applicationStatus;
  const userId = nestedFilters?.wfQuery?.assignee;
  ...
  return {
    ...
    processSearchCriteria: {
      ...
      ...(userId ? { assignee: userId === "ASSIGNED_TO_ME" ? USER_UUID : userId } : {}),
    },
 ...
 */
const InboxServiceApicall = async ({ tenantId, filters = {} }) => {
  try {
...
export default InboxServiceApicall;
