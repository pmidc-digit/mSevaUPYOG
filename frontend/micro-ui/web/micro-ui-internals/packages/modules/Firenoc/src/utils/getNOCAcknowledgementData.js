...
const getApplicantDetails = (appData, t) => {
  const owners = appData?.nocDetails?.additionalDetails?.applicationDetails?.owners != null ? appData.nocDetails.additionalDetails.applicationDetails.owners : [];
...
const getChecklistDetails = (appData, checklistData, t) => {
  ...
  const orderMap = {};
    sortedDocs?.forEach((doc, idx) => {
      orderMap[doc.uuid] = doc.order != null ? doc.order : idx + 1; // fallback to index
    });
