...
export const ApplicationCard = ({
  ...
  if (!data || data?.length === 0) {
    result = (
      <Card className="ads-components-inbox-application-card--style-1">
        {t("CS_MYAPPLICATIONS_NO_APPLICATION")
          .split("\\n")
          .map((text, index) => (
            <p key={index} className="ads-components-inbox-application-card--style-2">
              {text}
            </p>
          ))}
      </Card>
    ...
    result = <DetailsCard data={data} serviceRequestIdKey={serviceRequestIdKey} linkPrefix={linkPrefix ? linkPrefix : "/digit-ui/employee/ptr/"} />;
  }
