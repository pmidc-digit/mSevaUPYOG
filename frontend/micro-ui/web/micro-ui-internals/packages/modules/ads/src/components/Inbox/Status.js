...
const Status = ({ onAssignmentChange, searchParams, businessServices, statusMap, moduleCode }) => {
  ...
  const map = {
    "ADV.CREATE": t("ES_ADV_NEW_BOOKING"),
    "ADV.UPDATE": t("ES_ADV_UPDATE_BOOKING"),
  };

  const translateState = (state, t) => {
    return `${t(map[state.stateBusinessService])}` + " - " + t(`ES_ADS_COMMON_STATUS_${state.state || "CREATED"}`);
  ...
  return userRoleStates?.filter((e) => !e.isTerminateState).length ? (
    <div className="status-container">
      <div className="filter-label ads-components-inbox-status--style-1" >
        {t("ES_INBOX_STATUS")}
      </div>
      {userRoleStates
