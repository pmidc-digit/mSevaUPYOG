...
const ADSModal = ({
  ...
  const fromState = action?.fromStateCode || action?._rawStateObj?.state || null;
  const toState = action?.toStateCode || null;
  const statusToSend =
    action?.status || (preferToStateFor.has(action?.action) ? toState || fromState || action?.nextState : fromState || toState || action?.nextState);
  ...
  const { isLoading: financialYearsLoading, data: financialYearsData } = Digit.Hooks.pt.useMDMS(
    ...
    {},
    {
      details: {
        ...
        moduleDetails: [{ moduleName: "egf-master", masterDetails: [{ name: "FinancialYear", filter: "[?(@.module == 'ADV')]" }] }],
      },
    }
  );
  ...
  useEffect(() => {
    // set approvers or other metadata using workflowActions if needed
    if (workflowActions && workflowActions.length > 0) {
      // example: find current action metadata from workflowActions
      const meta = workflowActions.find((w) => w.action === action?.action);
      if (meta) {
        // use meta.assigneeRoles etc.
      }
    }
