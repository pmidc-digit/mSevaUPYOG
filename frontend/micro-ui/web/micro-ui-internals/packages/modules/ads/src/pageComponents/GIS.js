import React, { useState } from "react";
import { LocationSearchCard, LinkButton, Card } from "@mseva/digit-ui-react-components";
if (typeof window !== "undefined" && typeof window.process === "undefined") {
  window.process = { env: {} };
}

const GIS = ({ t, config, onSelect, formData = {}, handleRemove, onSave, props }) => {
  const [pincode, setPincode] = useState(formData?.address?.pincode || "");
  const [geoLocation, setGeoLocation] = useState(formData?.address?.geoLocation || {});
  const tenants = Digit.Hooks.obps.useTenants();
  const [pincodeServicability, setPincodeServicability] = useState(null);
  const [placeName, setPlaceName] = useState("");
  const tenantId = Digit.ULBService.getCurrentTenantId();
  const stateId = Digit.ULBService.getStateId();
  let Webview = !Digit.Utils.browser.isMobile();

  const onSkip = () => {
    // prefer onSelect if provided
    if (typeof onSelect === "function") {
      try {
        return onSelect();
      } catch (e) {
        console.warn("GIS: onSelect threw", e);
        return;
      }
    }

    // fallback to onSave if caller only provided onSave
    if (typeof onSave === "function") {
      try {
        // call with current geoLocation/pincode/placeName if available
        return onSave(geoLocation || {}, pincode || "", placeName || "");
      } catch (e) {
        // last-resort: call with single payload for older callers
        try {
          return onSave({ geoLocation: geoLocation || {}, pincode: pincode || "", placeName: placeName || "" });
        } catch (err) {
          console.warn("GIS: onSave also failed", err);
        }
      }
    }

    // nothing to call
    console.warn("GIS: neither onSelect nor onSave provided for skip");
  };
  const onChange = (code, location, place) => {
    setPincodeServicability(null);
    const foundValue = tenants?.find((obj) => obj.pincode?.find((item) => item == code));
    if (!foundValue) {
      setPincodeServicability("BPA_COMMON_PINCODE_NOT_SERVICABLE");
      setPincode("");
      setGeoLocation({});
      setPlaceName("");
    } else {
      setPincode(code);
      setGeoLocation(location);
      setPlaceName(place);
    }
  };

  return (
    <div
      className="ads-page-components-gis--style-1"
      onClick={(e) => {
        // prevent any clicks inside overlay from reaching parent forms
        if (e && e.preventDefault) e.preventDefault();
        if (e && e.stopPropagation) e.stopPropagation();
      }}
      onMouseDown={(e) => {
        if (e && e.preventDefault) e.preventDefault();
        if (e && e.stopPropagation) e.stopPropagation();
      }}
      onKeyDown={(e) => {
        // prevent Enter/Space from bubbling
        if (e && (e.key === "Enter" || e.key === " " || e.key === "Spacebar")) {
          if (e && e.preventDefault) e.preventDefault();
          if (e && e.stopPropagation) e.stopPropagation();
        }
      }}
    >
      <div className="ads-page-components-gis--style-2">

        <div className={Webview ? "ads-gis__location-search--web" : "ads-gis__location-search"}>
          <LocationSearchCard
            className="ads-page-components-gis--style-3"
            header={t ? t("ADS_GIS_LABEL") : "Pick location"}
            // explicit safe fallback for label text so button text is visible
            nextText={t ? t("ADS_PIN_LOCATION_LABEL") || "Pick location" : "Pick location"}
            skip={onSkip}
            t={t}
            position={geoLocation}
            // safe wrapper: call only if onSave exists
            onSave={() => {
              if (typeof onSave === "function") {
                try {
                  onSave(geoLocation, pincode, placeName);
                } catch (e) {
                  // fallback: call single-payload style if needed
                  try {
                    onSave({ geoLocation, pincode, placeName });
                  } catch (err) {
                    console.warn("GIS: onSave call failed", err);
                  }
                }
              } else {
                console.warn("GIS: onSave not provided");
              }
            }}
            onChange={(code, location, place) => {
              if (typeof onChange === "function") onChange(code, location, place);
            }}
            disabled={false}
            forcedError={pincodeServicability ? t(pincodeServicability) : null}
            isPlaceRequired={true}
            handleRemove={handleRemove}
            Webview={Webview}
            isPopUp={true}
          />
        </div>
      </div>
    </div>
  );
};

export default GIS;

/*
<svg style={{float:"left", position:"relative",bottom:"32px",marginTop:Webview?"62px":"57px",marginLeft:Webview?"26%":"20px"}} width="24" height="24" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M14.9999 9.66666C12.0533 9.66666 9.66658 12.0533 9.66658 15C9.66658 17.9467 12.0533 20.3333 14.9999 20.3333C17.9466 20.3333 20.3333 17.9467 20.3333 15C20.3333 12.0533 17.9466 9.66666 14.9999 9.66666ZM26.9199 13.6667C26.3066 8.10666 21.8933 3.69333 16.3333 3.07999V0.333328H13.6666V3.07999C8.10658 3.69333 3.69325 8.10666 3.07992 13.6667H0.333252V16.3333H3.07992C3.69325 21.8933 8.10658 26.3067 13.6666 26.92V29.6667H16.3333V26.92C21.8933 26.3067 26.3066 21.8933 26.9199 16.3333H29.6666V13.6667H26.9199ZM14.9999 24.3333C9.83992 24.3333 5.66658 20.16 5.66658 15C5.66658 9.83999 9.83992 5.66666 14.9999 5.66666C20.1599 5.66666 24.3333 9.83999 24.3333 15C24.3333 20.16 20.1599 24.3333 14.9999 24.3333Z" fill="#505A5F"/>
            </svg>
*/
