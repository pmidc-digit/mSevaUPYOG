...
import Stepper from "../../../../../react-components/src/customComponents/Stepper";
import { SiteInspectionConfig } from "../../config/Create/SiteInspectionConfig";
...
const createEmployeeConfig = [
  {
    ...
    stepLabel: "ES_TITILE_SITE_INSPECTION_DETAILS",
    stepNumber: 1,
    isStepEnabled: true,
    type: "component",
    component: "NewSiteInspectionStepperFormOne",
    key: "documents",
    withoutLabel: true,
    texts: {
...
const NewSiteInspectionStepperFormOne = ({ userType }) => {
  const config = createEmployeeConfig;
  const updatedConfig = config.map((item) => {
    return {
      ...item,
      currStepConfig: SiteInspectionConfig.filter((newConfigItem) => newConfigItem.stepNumber === item.stepNumber),
    };
  });
