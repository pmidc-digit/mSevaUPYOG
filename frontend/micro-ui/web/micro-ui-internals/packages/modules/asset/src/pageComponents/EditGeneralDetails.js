import { CardLabel, CardLabelError, CardCaption, LabelFieldPair, TextInput, Toast, Dropdown, TextArea } from "@mseva/digit-ui-react-components";
...
const EditGeneralDetails = ({ config, onSelect, formData, setError, clearErrors }) => {
  ...
  const { data: assetTypeData } = Digit.Hooks.useCustomMDMS(Digit.ULBService.getStateId(), "ASSET", [{ name: "AssetType" }], {
    select: (data) => {
  ...
  const { data: masterDropdown } = Digit.Hooks.useCustomMDMS(Digit.ULBService.getStateId(), "ASSET", [{ name: "ModeOfPossessionOrAcquisition" }], {
    select: (data) => {
