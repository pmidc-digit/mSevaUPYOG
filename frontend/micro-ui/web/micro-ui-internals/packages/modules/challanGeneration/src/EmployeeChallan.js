...
const EmployeeChallan = (props) => {
  ...
  const submitAction = (data) => {
    ...
    then((result) => {
      if (result.challans && result.challans.length > 0) {
        ...
        history.push(
          `/digit-ui/employee/challan-generation/acknowledgement?purpose=challan&status=success&tenantId=${challan?.tenantId}&serviceCategory=${challan.businessService}&challanNumber=${challan.challanNo}&applicationStatus=${challan.applicationStatus}`,
          { from: url }
        );
      }
    }).
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      ...
      billDetails &&
      billDetails.map((ob) => {
        if (ob.taxHeadCode.includes("CGST")) ob.order = 3;else
        if (ob.taxHeadCode.includes("SGST")) ob.order = 4;
      });
  ...
  return (
    <React.Fragment>
      <div className={"employee-application-details challan-generation-style-da303dcf86"}>
      ...
      <div>
        <Card>
          <StatusTable className="challan-generation-style-88bbba3a71">
            <Row label={`${t("UC_CHALLAN_NO")}`} text={challanno} />
            <hr className="challan-generation-style-791521f0b7" />
            ...
            <hr className="challan-generation-style-791521f0b7" />
            <Row
              label={<b className="challan-generation-style-88bbba3a71">{t("PAYMENT_CS_TOTAL_AMOUNT_DUE")}</b>}
              text={`₹${totalDueAmount}`}
              textStyle={{ fontSize: "24px", padding: "10px 0px", fontWeight: "700" }} />

          </StatusTable>
          <div className="challan-generation-style-6ffaa34fed">{t("SERVICEDETAILS")}</div>
          <StatusTable>
            <Row
          ...
          <div className="challan-generation-style-6ffaa34fed">{t("CONSUMERDETAILS")}</div>
          <StatusTable>
