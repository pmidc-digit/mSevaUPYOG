...
function NDCDocument({ value = {}, Code, index }) {
  ...
  return (
    <div className="challan-generation-style-8c313805d9">
      <React.Fragment>
        {data?.pdfFiles &&
        <div className="challan-generation-style-d3ebd854a3">
            {documents?.map((document, index) => {
            let documentLink = pdfDownloadLink(data.pdfFiles, document?.fileStoreId);
            return (
              <a target="_" href={documentLink} key={index} className="challan-generation-style-c0505f64f2">
                  <PDFSvg width={85} height={100} className="challan-generation-style-b9d23c1045" />
                </a>);
