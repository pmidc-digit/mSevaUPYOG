...
import { addQuestions, addQuestionsList, deleteCategory, updateCategory,updateMandatoryQuestionSelection, updateQuestionSelection, updateQuestionWeightage, setQuestions, goPrev } from '../../../redux/actions/surveyFormActions';
...
const CategoryCard = ({ category, checked, readOnly, onDelete, hideQuestionLabel }) => {
  ...
  const handleMandatoryQuestionSelection = (questionId, mandatory) => {
    ...
    console.log("quesupdate", category.id, questionId, mandatory)
    console.log("questionId", category.questions[0].uuid)
  ...
  const handleAddQuestions = () => {
    ...
    console.log("mq",mandatoryQuestions)
    ...
    const existingQuestions =  selectedQuestions.filter(question =>
      category.selectedQuestions.some(q => q.uuid === question.uuid)
    );

    console.log("exist qus",existingQuestions)
    console.log("selected qus",category.selectedQuestions)
    let index=null;
    ...
    console.log("selected questions",category.selectedQuestions)
    ...
    <div className="category-card">
      ...
      {((readOnly === false || readOnly === undefined)) && (
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px', marginBottom: '5px' }}>
          ...
          <label style={{
            ...
            color: "white",
            cursor: "pointer"
          }} onClick={() => setShowQuestionTable(!showQuestionTable)}>Add Questions</label>
          ...
          <label style={{
            ...
            // marginLeft:"5px",
            cursor: "pointer"
          }} onClick={() => { onDelete(category.id) }}>Delete Section</label>
      ...
      {(showQuestionTable || state.goPrev === true) && (
        <div>
          ...
          {(readOnly === false || readOnly === undefined) && (<label style={{
            padding: "10px 20px",
            border: "none",
            borderRadius: "4px",
            backgroundColor: "#007bff",
            color: "white",
            cursor: "pointer"
          }} disable={readOnly} onClick={handleGoClick}>Go</label>)}
