...
import { addCategory, deleteCategory, recalculateWeightage, updateWeightage } from '../../../redux/actions/surveyFormActions';
  ...
  return (
    <div className="create-survey-page" style={{ background: 'white', display: 'block', padding: '15px' }}>
      ...
      {categories.map(category => (
        <CategoryCard key={category.id} category={category} checked={checked} readOnly={readOnly} onDelete={handleDeleteCategory} hideQuestionLabel={hideQuestionLabel}/>
      ))}
        {readOnly!==true && (
      <label style={{
        padding: "10px 20px",
