...
import GCInboxFilters from "./GCInboxFilters";
import GCInboxSearch from "./GCInboxSearch";
...
const Inbox = ({ parentRoute }) => {
  ...
  function formReducer(state, payload) {
    ...
    Digit.SessionStorage.set("GC.INBOX", nextState);
    return nextState;
  }

  const storedInboxState = Digit.SessionStorage.get("GC.INBOX");
  ...
  const { isLoading: isInboxLoading, data } = Digit.Hooks.gc.useInbox({
    tenantId,
    filters: formState,
  ...
  const onSortingByData = useCallback(
    (sortingData) => {
      if (!sortingData.length) return;
      const [{ id, desc }] = sortingData;
      updateTableForm({ sortBy: id, sortOrder: desc ? "DESC" : "ASC" });
    },
    [updateTableForm]
  );
  ...
  return (
    <InboxWrapper
      ...
      tableHeader="GARBAGE_COLLECTION"
      filterSection={
        <GCInboxFilters
          statuses={statuses}
      ...
      topBar={<GCInboxSearch values={formState.searchForm} onSearch={handleSearch} />}
      pagination={
        <InboxPagination
