...
import { SET_GCNewApplication_STEP, RESET_GC_NEW_APPLICATION_FORM, UPDATE_GCNewApplication_FORM } from "../../redux/action/GCNewApplicationActions";
...
const NewPTRStepperForm = () => {
  ...
  useEffect(() => {
    ...
    if (id && applicationData?.PetRegistrationApplications?.length) {
      dispatch(UPDATE_GCNewApplication_FORM("responseData", applicationData.PetRegistrationApplications));
    }
  }, [applicationData, id, dispatch]);

  const setStep = (updatedStepNumber) => {
    dispatch(SET_GCNewApplication_STEP(updatedStepNumber));
  };
  ...
  useEffect(() => {
    const unlisten = history.listen(() => {
      // route changed
      dispatch(RESET_GC_NEW_APPLICATION_FORM());
