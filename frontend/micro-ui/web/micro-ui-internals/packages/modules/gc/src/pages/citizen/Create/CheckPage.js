...
const CheckPage = ({ onSubmit, value = {} }) => {
  ...
  const { applicant, adslist, index, isEditADS, isUpdateADS, documents, address } = value;
  ...
  const adslistRows =
    params?.adslist?.cartDetails.map((slot) => ({
        ...
        <div style={{ display: "flex", justifyContent: "space-between", width: "100%" }}>
          ...
          <CardSubHeader>
            <TimerValues
              ...
              SlotSearchData={params?.adslist?.cartDetails}
              draftId={params?.adslist?.existingDataSet?.draftId}
            />
          </CardSubHeader>
        </div>
        <div>
