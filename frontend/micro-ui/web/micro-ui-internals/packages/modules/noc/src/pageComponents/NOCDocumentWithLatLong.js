      ...
      {!isLoading ? (
        ...
        >
          ...
          {nocDocuments?.NOC?.Documents?.map((document, index) => {
            return (
              <div
                className="noc-page-components-nocdocument-with-lat-long--style-1"
              >
                <SelectDocument
                  key={index}
                  document={document}
    ...
    <div /* style={{ marginBottom: "24px" }} */>
      ...
      {doc?.code === "OWNER.SITEPHOTOGRAPHONE" &&
        (sessionStorage.getItem("Latitude") && sessionStorage.getItem("Longitude") ? (
          ...
          <div>
            <p className="noc-page-components-nocdocument-with-lat-long--style-2">Please upload a Photo with Location details.</p>
