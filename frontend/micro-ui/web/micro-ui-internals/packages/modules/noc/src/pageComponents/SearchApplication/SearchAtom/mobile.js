...
const SearchApplicationMobileView = ({ SearchFormFieldsComponent, propsMobileInboxCards, isLoading, data, getRedirectionLink, onSubmit }) => {
    ...
    return (
        ...
        {data?.[0]?.display ? (
            <Card className="noc-page-components-search-application-search-atom-mobile--style-1">
            {t(data?.[0]?.display)
                .split("\\n")
                .map((text, index) => (
                <p key={index} className="noc-page-components-search-application-search-atom-mobile--style-2">
                    {text}
                </p>
                ))}
            </Card>
