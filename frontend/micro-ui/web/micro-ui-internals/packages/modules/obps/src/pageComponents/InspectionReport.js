...
const InspectionReport = ({ config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  ...
  return (
    <div>
      <React.Fragment>
        {FieldReports && FieldReports.map((unit, index) => <InspectionReportForm key={unit.key} index={index} unit={unit} {...commonProps} />)}
      </React.Fragment>
  ...
  useEffect(() => {
    ...
    if (!_.isEqual(formValue, part)) {
      ...
      setFieldReports((prev) =>
        prev.map((o) => (o.key && o.key === unit.key ? { ...o, ...formValue, ..._ownerType, ...questionLength, ...Ques } : { ...o }))
      );
      trigger();
    }
  }, [formValue]);
          ...
          <LabelFieldPair style={{ width: "100%" }}>
            ...
            <div className="field" style={{ width: "100%" }}>
              <Controller
                ...
                control={control}
                render={(props) => <DatePicker date={props.value} name="InspectionDate" onChange={props.onChange} />}
              />
            </div>
          </LabelFieldPair>
          <LabelFieldPair style={{ width: "100%" }}>
            ...
            <div className="field" style={{ width: "100%" }}>
              <Controller
                ...
                rules={{ required: t("REQUIRED_FIELD") }}
                control={control}
                render={(props) => <TextInput name="InspectionTime" type="time" value={props.value} onChange={props.onChange} />}
              />
            </div>
          </LabelFieldPair>
          <CardSectionHeader>{t("BPA_CHECK_LIST_DETAILS")}</CardSectionHeader>
          {questionList &&
            questionList.map((ob, ind) => (
              <div key={ind} className="fieldInsepctionInsideWrapper" style={{ maxWidth: "100%" }}>
                ...
                <LabelFieldPair style={{ width: "100%" }}>
                  <CardLabel style={{ marginRight: "30px", width: "100%" }} className="card-label-smaller">
                    {t("BPA_ENTER_REMARKS")}
                  </CardLabel>
                  <div className="field" style={{ width: "100%" }}>
                    <Controller
