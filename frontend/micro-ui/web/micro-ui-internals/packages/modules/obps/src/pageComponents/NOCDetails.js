...
import { CardLabel, Dropdown, UploadFile, Toast, Loader, FormStep, StatusTable, MultiUploadWrapper, Row } from "@mseva/digit-ui-react-components";
...
const NOCDetails = ({ t, config, onSelect, userType, formData, setError: setFormError, clearErrors: clearFormErrors, formState }) => {
  ...
  let [beforeUploadNocDocuments, setBeforeUploadNocDocuments] = useState(
    formData?.PrevStateNocDocuments ? cloneDeep(formData?.PrevStateNocDocuments) : []
  );
  ...
  const { data: pdfDetails, isLoading: pdfLoading } = Digit.Hooks.useDocumentSearch(beforeUploadNocDocuments, {
    enabled: beforeUploadNocDocuments?.length > 0 ? true : false,
  });
...
function SelectDocument({
  ...
  const filteredDocument =
    nocDocuments?.filter((item) => item?.documentType?.includes(doc?.code))[0] ||
    beforeUploadNocDocuments?.filter((item) => item?.documentType?.includes(doc?.code))[0];
  ...
  const [uploadedFile, setUploadedFile] = useState(
    () =>
      ({
        fileStoreId: nocDocuments?.filter((item) => item?.documentType?.includes(doc?.code))[0]?.fileStoreId,
        fileName: nocDocuments?.filter((item) => item?.documentType?.includes(doc?.code))[0]?.fileName,
      } || null)
  );
  ...
  function getData(e, key) {
    ...
    if (e?.length > 0) {
      ...
      const __documents = [...nocDocuments.filter((e) => e.documentType !== key), ...newfiles];
  ...
  useEffect(() => {
    if (
      ...
      (nocDocuments.filter((ob) => ob.documentType === selectedDocument?.documentType).length == 0 ||
        newArray.filter((ob) => ob?.file?.documentType === selectedDocument?.documentType).length !==
          nocDocuments.filter((ob) => ob.documentType === selectedDocument?.documentType).length)
    ) {
  ...
  const uploadedFilesPreFill = useMemo(() => {
    ...
    formData?.nocDocuments?.nocDocuments
      ?.filter((ob) => ob.documentType === key)
      .forEach((e) =>
  ...
  return (
    ...
    >
      <div>
        <h1
          style={{ color: "#0B0C0C", lineHeight: "37px", fontWeight: "700", fontSize: "32px", fontFamily: "Roboto Condensed", paddingBottom: "24px" }}
        >
          {t(`BPA_${doc?.nocType}_HEADER`)}
        </h1>
        <div style={{ display: "flex", paddingBottom: "24px" }}>
          <h1 style={{ color: "#0B0C0C", lineHeight: "19px", fontWeight: "700", fontSize: "16px", fontFamily: "Roboto", marginRight: "10px" }}>
            {t(`BPA_${doc?.nocType}_LABEL`)}
          </h1>
          <h1>{doc?.additionalDetails?.appNumberLink}</h1>
        </div>
      </div>
      {!window.location.href.includes("sendbacktocitizen") && (
        <MultiUploadWrapper
          module="BPA"
          tenantId={tenantId}
      ...
      {doc?.uploadedDocuments?.length && !pdfLoading && (
        <DocumentsPreview
