...
const NOCNumber = ({ t, config, onSelect, userType, formData, setError: setFormError, clearErrors: clearFormErrors, formState }) => {
    ...
    if (!formData?.id) {
      ...
      if (
        ...
        if (formData?.data.edcrDetails.planDetail.blocks[0].building.buildingHeight > 15) {
          ...
          formData?.data.edcrDetails.planDetail.planInformation.plotArea > 500
        ) {
          ...
          formData?.data.edcrDetails.planDetail.planInformation.plotArea > 250
        ) {
  ...
  return (
    <div>
      ...
      <FormStep t={t} config={config} onSelect={handleSubmit} isDisabled={!nocNumber} onChange={onChange}>
        <span
          onClick={onClick}
          style={{
