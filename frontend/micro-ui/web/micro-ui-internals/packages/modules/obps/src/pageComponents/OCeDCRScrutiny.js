...
const OCeDCRScrutiny = ({ t, config, onSelect, userType, formData, ownerIndex = 0, addNewOwner, isShowToast }) => {
      ...
      .then((result, err) => {
        if (result?.BPA?.length > 0) {
          ...
          if (permitDate === convertEpochToDate(result?.BPA?.[0]?.approvalDate)) {
            Digit.OBPSService.scrutinyDetails(tenantIdForEdcr, { edcrNumber: edcrNumber })
              .then((response) => {
                Digit.WorkflowService.getAllApplication(tenantIdForEdcr, { businessIds: result?.BPA?.[0]?.applicationNo, history: true })
                  .then((workFlowData, err) => {
                    ...
                    if (response?.edcrDetail?.length > 0) {
                      const isPrimaryOwner = result?.BPA?.[0]?.landInfo?.owners?.filter(
                        (data) => data.isPrimaryOwner && data.isPrimaryOwner != "false"
                      );
  ...
  return (
    <div>
      <FormStep t={t} config={config} onSelect={handleSubmit} onSkip={onSkip} isDisabled={!permitNumber} onAdd={onAdd} isMultipleAllow={true}>
        ...
        <DatePicker date={permitDate} name="permitDate" onChange={setOCPermitDate} />
      ...
      {permitEdcrData?.edcrNumber ? (
        <FormStep t={t} config={config} onSelect={handleSubmit} onSkip={onSkip} isDisabled={!permitNumber} onAdd={onAdd} isMultipleAllow={true}>
          <StatusTable>
            <Row label={`${t("REPORT_FSM_RESULT_APPLICANTNAME")}`} text={t(permitEdcrData?.applicantName)} className="border-none" />
            <Row label={`${t("BPA_BASIC_DETAILS_SERVICE_TYPE_LABEL")}`} text={t(permitEdcrData?.applicationSubType)} className="border-none" />
            <Row
            ...
            <Row
              ...
              className="border-none"
            />
            <Row label={`${t("BPA_ARCHITECT_NAME_LABEL")}`} text={t(permitEdcrData?.appliedBy)} className="border-none" />
          </StatusTable>
          <hr style={{ color: "#cccccc", backgroundColor: "#cccccc", height: "2px", marginTop: "20px", marginBottom: "20px" }} />
          <h1 style={{ fontSize: "18px", lineHeight: "21px", fontWeight: "700", padding: "0px 0px 16px 0px" }}>{`${t(
            "BPA_PERMIT_EDCR_DETAILS_LABEL"
          )}`}</h1>
          <StatusTable>
            <Row label={`${t("BPA_EDCR_NO_LABEL")}`} text={t(permitEdcrData?.edcrNumber)} className="border-none" />
          </StatusTable>
          <h1 style={{ fontSize: "18px", lineHeight: "21px", fontWeight: "700", padding: "0px 0px 16px 0px" }}>{`${t(
            "BPA_UPLOADED_PLAN_DIAGRAM_LABEL"
          )}`}</h1>
          <a target="_" href={permitEdcrData?.updatedDxfFile} style={{ minWidth: "160px", marginRight: "20px" }} key={permitEdcrData?.updatedDxfFile}>
            ...
            <PDFSvg />
            <p style={{ marginTop: "8px", fontWeight: "bold", fontSize: "14px", lineHeight: "19px", color: "#505A5F", fontWeight: "400" }}>
              {t("BPA_UPLOADED_PLAN_DXF")}
            </p>
          </a>
          <h1 style={{ fontSize: "18px", lineHeight: "21px", fontWeight: "700", padding: "0px 0px 16px 0px" }}>{`${t(
            "BPA_SCRUTINY_REPORT_OUTPUT_LABEL"
          )}`}</h1>
