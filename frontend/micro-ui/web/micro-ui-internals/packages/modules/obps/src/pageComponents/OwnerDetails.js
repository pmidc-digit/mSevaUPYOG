    ...
    if (moveforward) {
      if (ismultiple == true && fields.length == 1) {
        ...
        if (!formData?.id) {
          ...
          // create BPA call
          if (
            isSelfCertificationRequired === true &&
            formData?.data.occupancyType === "Residential" &&
            (parsedArchitectName == "ARCHITECT" ||
              parsedArchitectName == "ENGINEER" ||
              parsedArchitectName == "DESIGNER" ||
              parsedArchitectName == "SUPERVISOR")
          ) {
            ...
            //converting plot area which is in square meters to square yards by multiplying 1.196
              ...
              (parsedArchitectName == "ARCHITECT" || parsedArchitectName == "ENGINEER") &&
              formData?.data.edcrDetails.planDetail.planInformation.plotArea * 1.19599 > 500
            ) {
              alert("Architect/Engineer can apply for area less then 500 sq. yards. in self declaration");
            } else if (
              (parsedArchitectName == "DESIGNER" || parsedArchitectName == "SUPERVISOR") &&
              formData?.data.edcrDetails.planDetail.planInformation.plotArea * 1.19599 > 250
            ) {
              ...
              return (
                <div key={`${field}-${index}`}>
                  ...
                  >
                    ...
                    <CardLabel>{`Upload Valid ID Copy (PAN/Voter ID/ Driving License) (PDF, Max 5MB) *`}</CardLabel>
                    <UploadFile
                    ...
                    <CardLabel>{`Upload Owner Photo *`}</CardLabel>
                    <UploadFile
...
//   const ownerRoleCheck = {}; // Declare the ownerRoleCheck variable
...
//       if (!field.name) newErrors[`name_${index}`] = "Name is required";
//       if (!field.mobileNumber) newErrors[`mobile_${index}`] = "Mobile number is required";
//       if (!field.gender) newErrors[`gender_${index}`] = "Gender is required";
//       // if (!field.documentType) newErrors[`documentType_${index}`] = "Document type is required";
//       if (!field.documentFile) newErrors[`documentFile_${index}`] = "Document upload is required";
//       if (!field.ownerPhoto) newErrors[`ownerPhoto_${index}`] = "Owner photo is required";
//       if (!field.dateOfBirth) newErrors[`dob_${index}`] = "Date of birth is required";
//       if (!field.ownerAddress) newErrors[`address_${index}`] = "Owner address is required";
//       if (field.authorizedPerson && !field.authorizationLetter) {
//         newErrors[`authLetter_${index}`] = "Authorization letter is required when authorized person is specified";
//       }
//     });

//     if (!ownershipCategory) newErrors.ownershipCategory = "Ownership category is required";

//     setErrors(newErrors);
//     return Object.keys(newErrors).length === 0;
//   };

//   const setDocumentType = (index, value) => {
//     const updatedFields = [...fields];
//     updatedFields[index].documentType = value;
//     setFeilds(updatedFields);
//   };
...
//   function setOwnerName(index, e) {
//     const units = [...fields];
//     units[index].name = e.target.value;
//     setName(e.target.value);
//     setFeilds(units);
//     if (units[index].gender && units[index].mobileNumber && units[index].name) {
//       setCanmovenext(false);
//     }
//   }

//   function setGenderName(index, value) {
//     const units = [...fields];
//     units[index].gender = value;
//     setGender(value);
//     setFeilds(units);
//     if (units[index].gender && units[index].mobileNumber && units[index].name) {
//       setCanmovenext(false);
...
//   function setOwnerEmail(index, e) {
//     const units = [...fields];
//     units[index].emailId = e.target.value;
//     setEmail(e.target.value);
//     setFeilds(units);
//     if (units[index].gender && units[index].mobileNumber && units[index].name) {
//       setCanmovenext(false);
...
//   // function setAadharNumber(index, e) {
//   //   const units = [...fields];
//   //   units[index].aadharNumber = e.target.value;
//   //   setaadharNumber(e.target.value);
//   //   setFeilds(units);
//   //   if (units[index].gender && units[index].mobileNumber && units[index].name) {
//   //     setCanmovenext(false);
...
//   function setMobileNo(index, e) {
//     const units = [...fields];
//     units[index].mobileNumber = e.target.value;
//     setMobileNumber(e.target.value);
//     setFeilds(units);
//     if (units[index].gender && units[index].mobileNumber && units[index].name) {
//       setCanmovenext(false);
...
//   function setPrimaryOwner(index, e) {
//     const units = [...fields];
//     units.map((units) => {
//       units.isPrimaryOwner = false;
//     });
//     units[index].isPrimaryOwner = ismultiple ? e.target.checked : true;
//     setisPrimaryOwner(e.target.checked);
//     setFeilds(units);
//   }
...
//     } else {
//       const usersResponse = await Digit.UserService.userSearch(Digit.ULBService.getStateId(), { userName: fields?.[indexValue]?.mobileNumber }, {});
//       const found = usersResponse?.user?.[0]?.roles?.filter((el) => el.code === "BPA_ARCHITECT" || el.code === "BPA_SUPERVISOR")?.[0];
//       if (usersResponse?.user?.length === 0) {
//         setShowToast({ key: "true", warning: true, message: "ERR_MOBILE_NUMBER_NOT_REGISTERED" });
//         return;
//       } else {
...
//   const goNext = () => {
//     if (!validateFields()) {
//       setShowToast({ key: "true", error: true, message: "Please fill all required fields" });
//       return;
//     }
...
//       ownerStep = { ...owner, ownershipCategory, owners: fields };
//     } else {
//       ownerStep = { ...owner, ownershipCategory, owners: fields };
//     }
//     onSelect(config.key, ownerStep);
...
//                         </div>
//                       </div>
//                       <ErrorMessage message={errors[`mobile_${index}`]} />
//                     </div>
//                     <CardLabel>{`${t("CORE_COMMON_NAME")} *`}</CardLabel>
//                     <TextInput
//                       style={{ background: "#FAFAFA" }}
...
//                     <CardLabel>{`Upload Document (PDF, Max 5MB) *`}</CardLabel>
//                     <UploadFile
//                       id={`document-upload-${index}`}
//                       onUpload={selectDocumentFile(index)}
//                       onDelete={() => {
