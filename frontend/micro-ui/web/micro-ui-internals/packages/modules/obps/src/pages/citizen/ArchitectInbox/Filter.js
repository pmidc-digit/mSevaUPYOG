...
const Filter = ({ searchParams, paginationParms, onFilterChange, onSearch, onClose, removeParam, statuses, ...props }) => {
  ...
  const filteredStatus = useMemo(
    () =>
      _searchParams?.applicationType?.length > 0 && _searchParams.businessService
        ? statuses?.filter((e) => {
        ...
        : null,
    [statuses, _searchParams.businessService, _searchParams?.applicationType]
  );
  ...
  const handleChange = (option, typeOfValue) => {
    if (typeOfValue == "APPLICATION_TYPE")
      setSearchParams({ ...searchParams, applicationType: option?.applicationType, applicationStatus: [], businessService: [] });
    if (typeOfValue == "RISK_TYPE")
      setSearchParams({
        ...searchParams,
        applicationType: _searchParams?.applicationType,
        applicationStatus: [],
        businessService: option?.businessService,
      });
  };
    ...
    <React.Fragment>
      <div className="filter">
        <div className="filter-card">
          ...
          {availableBusinessServicesOptions?.length > 0 ? (
            <div>
