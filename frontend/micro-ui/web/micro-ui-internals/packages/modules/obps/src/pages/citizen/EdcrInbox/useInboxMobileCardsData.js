...
const useInboxMobileCardsData = ({  table }) => {
  ...
  const dataForMobileInboxCards = table?.map(
    ({ applicationId, edcrNumber, date, planReportUrl, dxfFileurl, locality, status, owner }) => ({
      ...
      [t("EDCR_COMMON_TABLE_COL_STATUS")]: (status === "Accepted" ? <span className="sla-cell-success  obps-pages-citizen-edcr-inbox-use-inbox-mobile-cards-data--style-1" >{status}</span> : <span className="sla-cell-error obps-pages-citizen-edcr-inbox-use-inbox-mobile-cards-data--style-2" >{status}</span>),
