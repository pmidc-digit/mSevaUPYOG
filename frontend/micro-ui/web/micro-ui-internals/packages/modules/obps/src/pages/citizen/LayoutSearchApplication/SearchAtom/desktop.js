  ...
  const TableComponent = () => {
    if (isLoading) {
      ...
      return data?.[0]?.display ? (
        <Card className="obps-pages-citizen-layout-search-application-search-atom-desktop--style-1">
          {t(data?.[0]?.display)
            .split("\\n")
            .map((text, index) => (
              <p key={index} className="obps-pages-citizen-layout-search-application-search-atom-desktop--style-2">
                {text}
              </p>
            ))}
        </Card>
