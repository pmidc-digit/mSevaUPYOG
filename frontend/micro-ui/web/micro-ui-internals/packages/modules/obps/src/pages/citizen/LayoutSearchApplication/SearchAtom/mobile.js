...
const SearchApplicationMobileView = ({ SearchFormFieldsComponent, propsMobileInboxCards, isLoading, data, getRedirectionLink, onSubmit }) => {
    ...
    return (
        ...
        {data?.[0]?.display ? (
            <Card className="obps-pages-citizen-layout-search-application-search-atom-mobile--style-1">
            {t(data?.[0]?.display)
                .split("\\n")
                .map((text, index) => (
                <p key={index} className="obps-pages-citizen-layout-search-application-search-atom-mobile--style-2">
                    {text}
                </p>
                ))}
            </Card>
            ...
            {...{
                ...
                linkPrefix: `/digit-ui/citizen/obps/layout/application-overview/`,
