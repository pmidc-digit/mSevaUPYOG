...
const OBPSAcknowledgement = ({ data, onSuccess }) => {
  ...
  const mutation = Digit.Hooks.obps.useObpsAPI(data?.tenantId ? data?.tenantId : tenantId, true);
  const mutation1 = Digit.Hooks.obps.useObpsAPI(data?.tenantId ? data?.tenantId : tenantId, false);
