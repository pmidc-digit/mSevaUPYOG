import { Banner, Card, CardText, ActionBar, SubmitBar, Loader, LinkButton } from "@mseva/digit-ui-react-components";
...
const OBPSResponse = (props) => {
  ...
  const getHeaderMessage = () => {
    return t(
      `BPA_HEADER_${bpaBusinessService}_${bpaData?.workflow?.action}_${stringReplaceAll(bpaStatus, " ", "_").toUpperCase()}${
        isSanctionFee ? isSanctionFee : ""
      }`
    );
  };
  ...
  const getSubHeaderMessage = () => {
    return t(
      ...
      }_${stringReplaceAll(bpaStatus, " ", "_").toUpperCase()}${isSanctionFee ? isSanctionFee : ""}`
    );
  };
  ...
  return (
    <div>
      {isLoader || workflowDetails?.isLoading ? (
        <Loader />
      ) : (
        <Card>
          <Banner
