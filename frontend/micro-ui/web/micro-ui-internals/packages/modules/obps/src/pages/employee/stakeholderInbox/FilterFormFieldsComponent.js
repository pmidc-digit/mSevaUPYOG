...
const FilterFormFieldsComponent = ({
  ...
  const { data: stakeholderServiceTypes, isLoading: stakeholderServiceTypesLoading } = Digit.Hooks.obps.useMDMS(
    stateId,
    ...
    <>
      ...
      <FilterFormField>
        <Controller
          ...
          control={controlFilterForm}
          render={(props) => {
            return stakeholderServiceTypesLoading ? (
              <Loader />
            ) : (
              <>
      ...
      {selectedBusinessService ? (
        <FilterFormField>
          <div className="filter-label" style={{ fontSize: "18px", fontWeight: "600" }}>
          ...
          <Controller
            ...
            render={(props) => {
              ...
              const renderStatusCheckBoxes = useMemo(
                () =>
                  statuses
                    ?.filter((e) => e.businessservice === selectedBusinessService.identifier)
                    ?.map((status, index) => {
                      return (
                        <CheckBox
                          key={index}
              ...
              return <>{isInboxLoading ? <Loader /> : <>{renderStatusCheckBoxes}</>}</>;
            }}
          />
        </FilterFormField>
      ) : null}
