...
//   // )}`;
//     if (data.address && data.address.locality && data.landInfo?.address?.tenantId) {
//     data.address.locality["i18nkey"] = `${t(
//       `${stringReplaceAll(data?.landInfo?.address?.tenantId, ".", "_").toUpperCase()}_REVENUE_${data?.landInfo?.address?.locality?.code}`,
//     )}`
//   }
...
export const getBPAFormData = async (data, mdmsData, history, t) => {
  ...
  if (!data.landInfo) {
    console.warn("landInfo is null - proceeding with default values")
    // Set default values when landInfo is null
    data.landInfo = {
      unit: [],
      owners: [],
      address: {
  ...
  if (
    ...
    data.landInfo?.address?.tenantId &&
    data.landInfo?.address?.locality?.code
  ) {
    data.address.locality["i18nkey"] = `${t(
      `${stringReplaceAll(data?.landInfo?.address?.tenantId, ".", "_").toUpperCase()}_REVENUE_${data?.landInfo?.address?.locality?.code}`,
    )}`
  ...
  if (data?.landInfo?.owners && Array.isArray(data.landInfo.owners)) {
    data?.landInfo.owners.map((owner, ind) => {
      owner.gender = {
        active: true,
        code: owner.gender,
  ...
  const formData = {
    BPA: {
      ...
      additionalDetails: {
        ...
        GISPlaceName: data?.address?.placeName,
        holdingNo: data?.data?.holdingNumber ? data?.data?.holdingNumber : data?.additionalDetails?.holdingNo,
        boundaryWallLength: data?.data?.boundaryWallLength
          ? data?.data?.boundaryWallLength
          : data?.additionalDetails?.boundaryWallLength,
        registrationDetails: data?.data?.registrationDetails
          ? data?.data?.registrationDetails
          : data?.additionalDetails?.registrationDetails,
        architectconsentdocument: {
          documentType: "Architect Consent Form",
          fileStoreId: sessionStorage.getItem("ArchitectConsentform"),
          fileStore: sessionStorage.getItem("ArchitectConsentform"),
...
export const getBPAUnit = (data) => {
  ...
  if (ob) {
    ...
    result &&
      result.map((newunit, index) => {
        const found = units.length > 0 ? units?.some((el) => el?.floorNo === newunit[0]?.split("_")[1]) : false
        if (!found) {
          units.push({
            blockIndex: index,
            floorNo: newunit[0]?.split("_")[1],
...
export const getBPAOwners = (data, isOCBPA) => {
  if (isOCBPA) return data.landInfo.owners
  const bpaownerarray = cloneDeep(data?.owners?.owners)
  bpaownerarray &&
    bpaownerarray?.forEach((newOwner) => {
      if (newOwner?.gender?.code) newOwner.gender = newOwner.gender?.code
      if (!newOwner?.fatherOrHusbandName) newOwner.fatherOrHusbandName = "NAME"
  ...
  data?.landInfo?.owners?.map((oldowner) => {
    const found = bpaownerarray?.length > 0 ? bpaownerarray?.some((el) => el.id === oldowner.id) : false
    if (!found) bpaownerarray?.push({ ...oldowner, active: false })
  })
...
export const downloadAndPrintReciept = async (
  ...
  const fileStore = await Digit.PaymentService.printReciept(tenantId, { fileStoreIds: response.filestoreIds[0] })
  window.open(fileStore[response?.filestoreIds[0]], "_blank")
  const responseStatus = Number.parseInt(response.status, 10)
  if (responseStatus === 201 || responseStatus === 200) {
    const fileName =
      mode == "print"
        ? printPdf(new Blob([response.data], { type: "application/pdf" }))
...
export const getOrderedDocs = (docs) => {
  ...
  const uniqueDocs = docs.filter(
    (elem, index) => docs.findIndex((obj) => obj.documentType === elem.documentType) === index,
  )
  uniqueDocs.map((ob) => {
    ob["filestoreIdArray"] = []
    docs
...
export const scrutinyDetailsData = async (edcrNumber, tenantId) => {
  ...
  const bpaDetails = await Digit.OBPSService.BPASearch(tenantId, { edcrNumber: edcrNumber })
  if (bpaDetails?.BPA?.length == 0) {
    return scrutinyDetails?.edcrDetail?.[0]
      ? scrutinyDetails?.edcrDetail?.[0]
      : { type: "ERROR", message: "BPA_NO_RECORD_FOUND" }
  } else if (
    bpaDetails?.BPA?.length > 0 &&
...
export const ocScrutinyDetailsData = async (edcrNumber, tenantId) => {
  ...
  const bpaDetails = await Digit.OBPSService.BPASearch(tenantId, {
    approvalNo: scrutinyDetails?.edcrDetail?.[0]?.permitNumber,
  })
  const bpaEdcrNumber = bpaDetails?.BPA?.[0]?.edcrNumber
  tenantId = bpaDetails?.BPA?.[0]?.tenantId
  const edcrDetails = await Digit.OBPSService.scrutinyDetails(tenantId, { edcrNumber: bpaEdcrNumber })
...
export const getOrderDocuments = (appUploadedDocumnets, isNoc = false) => {
  const finalDocs = []
  if (appUploadedDocumnets?.length > 0) {
    const uniqueDocmnts = appUploadedDocumnets.filter(
      (elem, index) =>
        appUploadedDocumnets.findIndex(
          (obj) =>
            obj?.documentType?.split(".")?.slice(0, 2)?.join("_") ===
            elem?.documentType?.split(".")?.slice(0, 2)?.join("_"),
        ) === index,
    )
    uniqueDocmnts?.map((uniDoc) => {
      const resultsDocs = appUploadedDocumnets?.filter(
        (appDoc) =>
          uniDoc?.documentType?.split(".")?.slice(0, 2)?.join("_") ==
          appDoc?.documentType?.split(".")?.slice(0, 2)?.join("_"),
      )
      resultsDocs?.forEach((resDoc) => (resDoc.title = resDoc.documentType))
      finalDocs.push({
        title: !isNoc ? resultsDocs?.[0]?.documentType?.split(".")?.slice(0, 2)?.join("_") : "",
