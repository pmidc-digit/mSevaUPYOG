...
export const getBPAFormData = async (data, mdmsData, history, t) => {
  ...
  if (!data.landInfo) {
    console.warn("landInfo is null/undefined - creating default structure")
    data.landInfo = {
      unit: [],
      owners: [],
      address: {
  ...
  data.BlockIds = getBlockIds(data?.landInfo?.unit || [])
  data.address = data?.landInfo?.address || { tenantId: data?.tenantId || "", locality: { code: "" } }

  try {
    data.address.locality["i18nkey"] = `${t(
      `${stringReplaceAll(data?.landInfo?.address?.tenantId || data?.tenantId || "", ".", "_").toUpperCase()}_REVENUE_${data?.landInfo?.address?.locality?.code || ""}`,
    )}`
  } catch (error) {
    console.warn("Could not set locality i18nkey:", error)
  }
  ...
  try {
    ...
    console.warn("Error processing owners:", error)
    data.owners = {
      owners: [],
      ownershipCategory: data?.ownershipCategory?.code,
    }
