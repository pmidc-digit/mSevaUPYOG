...
const Inbox = ({
  ...
  const [TableConfig, setTableConfig] = useState(() => Digit.ComponentRegistryService?.getComponent("PGRAIInboxTableConfig"));
  // const [getSearchFi]
    ...
    if (isMobile) {
      return (
        <MobileInbox
          ...
          linkPrefix={`${parentRoute}/complaint-details/`}
          tableConfig={rest?.tableConfig ? rest?.tableConfig:TableConfig(t)["PGRAI"]}
      ...
      return (
        <div>
          ...
          <DesktopInbox
            moduleCode={moduleCode}
            data={data}
            tableConfig={TableConfig(t)["PGRAI"]}
            isLoading={hookLoading}
