...
const PTCreateDocsRequired = ({ onSelect, userType, onSkip, config }) => {
    ...
    const { isLoading: ptDocsLoading, data: ptDocs } = Digit.Hooks.pt.usePropertyMDMS(stateId, "PropertyTax", ["Documents"]);
    console.log("ptDocs: ", ptDocs);
    //   console.log("ws /create-application", ptDocs, ptDocsLoading);
    ...
    //           ) : (
    //             <Fragment>
    //               {ptDocs?.Documents?.map((doc, index) => (
    //                 <div>
    ...
    const printDiv = () => {
        const content = `<h1>${t("PT_SERVICES_MASTERS_REQ_DOCS_HEADER")}</h1>` + document.getElementById("documents-div").innerHTML;
        ...
        <div className="pageCard">
            <div>
                <Header styles={{ fontSize: "32px", marginLeft: "18px", display: "flex", justifyContent: "space-between", marginRight: "12px" }}>
                    <div>{t("PT_SERVICES_MASTERS_REQ_DOCS_HEADER")}</div>
            ...
            <Card>
                {ptDocsLoading ? (
                    ...
                    <div id="documents-div">
                        {ptDocs?.PropertyTax?.Documents?.map((doc, index) => (
                            doc.active &&
                            <div key={index} style={{ marginTop: "16px" }}>
                                <CardSectionHeader style={{ marginBottom: "16px", lineHeight: "28px", fontSize: "24px" }}>
                                ...
                                <div style={{ margin: "16px 0", lineHeight: "18px", fontSize: "16px" }}>
                                    {doc?.dropdownData?.map((value, idx) => ((
                                        value.active &&
                                        <p style={{ fontWeight: "bold", lineHeight: "32px" }}>{`${idx + 1}. ${t(value?.code?.replaceAll(".","_"))}`}</p>)
                                    ))}
                                </div>
                                <p style={{ fontSize: "16px" }}>{t(`${doc?.description.replaceAll(".", "_")}`)}</p>
                            </div>
                        ))}
                    </div>
                )}
                <ActionBar style={{ display: "flex", justifyContent: "flex-end", alignItems: "baseline" }}>
                    {
                        <SubmitBar
                            label={t("Next")}
                            onSubmit={() => {
                                history.push(match.path.replace("new-application", "create-application"));
                            }}
                            style={{ margin: "10px 10px 0px 0px" }}
                            disabled={ptDocsLoading ? true : false}
                        />
                    }
                </ActionBar>
            </Card>
...
export default PTCreateDocsRequired;
