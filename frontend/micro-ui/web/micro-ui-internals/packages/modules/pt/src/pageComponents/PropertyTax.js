...
const PropertyTax = ({ t, config, onSelect, userType, formData }) => {
  ...
  //   if(isLoading===false)
  //   {
  //      docs = Documentsob?.PropertyTax?.[config?.isMutation ? docType[0] : docType];
  //   }
  //     },[!isLoading])
  ...
  useEffect(() => {
    if (Documentsob) {
      const docObj = Documentsob?.PropertyTax?.[config?.isMutation ? docType[0] : docType];
      setDocs(docObj)
    }
  }, [Documentsob]);
  ...
  return (
    ...
    { showToast && 
     <Modal
          headerBarMain={<Heading label={"Required Documents - Property Tax (New Property should be created if u have never paid tax against that property. Otherwise use same Property Id to pay tax.)"} />}
              ...
              (config.isMutation) ?
                ...
                docs.map(({ code, dropdownData }, index) => (
                  <div key={index}>
                    ...
                    <CardText className={"primaryColor"}>One of these documents is needed to apply for this Service</CardText>
                    {dropdownData.map((dropdownData) => (
                      <CardText className={"primaryColor"}>{t("PROPERTYTAX_" + stringReplaceAll(dropdownData?.code, ".", "_") + "_LABEL")}</CardText>
                    ))}
                     <CardText className={"primaryColor"}>* In case of multiple/institutional Applicant please provide ID of primary or authorized person</CardText>
                  </div>
                ))
              : null}
            {/* // : null} */}
