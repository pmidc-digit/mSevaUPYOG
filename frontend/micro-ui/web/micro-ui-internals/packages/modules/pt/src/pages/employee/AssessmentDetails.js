...
const AssessmentDetails = () => {
  ...
  console.log("Assessment1", location);
  console.log("Assessment2", location.state);
  console.log("Assessment3", location.state?.Assessment);
  ...
  console.log("assessmentMutate", assessmentMutate);
  ...
  const handleAssessment = () => {
    if (!queryClient.getQueryData(["PT_ASSESSMENT", propertyId, location?.state?.Assessment?.financialYear])) {
      const AssessmentData = {
        ...
        financialYear: location?.state?.Assessment?.financialYear || "2025-26",
        assessmentDate: Date.now(), // or use the correct value
  ...
  console.log("address to display", address_to_display);
  ...
  function change() {
    ...
    if (selectedPenalityReason && first > 0) {
      if (selectedPenalityReason.value !== "Others") {
        if (first < total_amount) {
          updatedDetails.adhocPenalty = first;
          updatedDetails.adhocPenaltyReason = selectedPenalityReason.value;
        } else {
          alert("Penality cannot exceed total amount");
        }
      } else {
        if (first < total_amount) {
          updatedDetails.adhocPenalty = first;
          updatedDetails.adhocPenaltyReason = fourth_temp.current.value;
        } else {
          alert("Penality cannot exceed total amount");
        }
      }
    ...
    // Rebate logic
    if (selectedRebateReason && second > 0) {
      if (selectedRebateReason.value !== "Others") {
        if (second < total_amount) {
          updatedDetails.adhocExemption = second;
          updatedDetails.adhocExemptionReason = selectedRebateReason.value;
        } else {
          alert("Adhoc Exemption cannot be greater than the estimated tax for the given property");
        }
      } else {
        if (second < total_amount) {
          updatedDetails.adhocExemption = second;
          updatedDetails.adhocExemptionReason = third_temp.current.value;
        } else {
          alert("Adhoc Exemption cannot be greater than the estimated tax for the given property");
        }
      }
        ...
        <Modal
          ...
          headerBarEnd={
            <CloseBtn
              onClick={() => {
                showPopUp(false);
                if (ptCalculationEstimateData && ptCalculationEstimateData.Calculation) {
