...
import { ActionBar, Header, Loader, SubmitBar, Card, CardSubHeader, CardSectionHeader, LinkLabel, CardLabel, CardHeader, CardText } from "@mseva/digit-ui-react-components";
...
const AssessmentDetails = () => {
  ...
  // Derive shared sections from hook data: full address, full property details, and owner details
  const addressSection = applicationDetails?.applicationDetails?.find((s) => s.title === "PT_PROPERTY_ADDRESS_SUB_HEADER");
  const propertyDetailsSection = applicationDetails?.applicationDetails?.find((s) => s.title === "PT_ASSESMENT_INFO_SUB_HEADER");
  const ownerSection = applicationDetails?.applicationDetails?.find((s) => s.title === "PT_OWNERSHIP_INFO_SUB_HEADER");
  ...
  return (
    <div>
      <Header>{t("Application Summary")}</Header>
        ...
        applicationDetails={
          {
            applicationDetails: [
                ...
                belowComponent: () => (
                  <>
                    <div style={{ display: "flex", gap: "24px", flexWrap: "wrap", alignItems: "center" }}>
                      <LinkLabel onClick={() => { showPopUp(true) }} style={isMobile ? { color: "#a82227", marginLeft: "0px" } : { color: "#a82227" }}>{t("PT_ADD_REBATE_PENALITY")}</LinkLabel>
                      {ptCalculationEstimateData?.Calculation?.length > 0 && (
                        <LinkLabel onClick={() => { setShowCalc(true); }} style={isMobile ? { color: "#a82227", marginLeft: "0px" } : { color: "#a82227" }}>{t("CALCULATION DETAILS")}</LinkLabel>
                      )}
                    </div>
                    <PTImportantDates financialYear={location?.state?.Assessment?.financialYear} />
                  </>
                )
              },
              // Property Address — full address fields from hook (City, House No, Colony, Street, Mohalla, Pincode, Existing PID, Survey ID, Year of Creation)
              ...(addressSection ? [addressSection] : []),
              // Property Details — full 12-field version from hook (incl. Vasika, Allotment, Business Name, etc.) with floor units
              ...(propertyDetailsSection ? [propertyDetailsSection] : [
                {
                  ...
                  additionalDetails: {
                    floors: applicationDetails?.applicationData?.units
                      ...
                      ?.map((unit, index) => {
                        const values = [
                          { title: `${t("ES_APPLICATION_DETAILS_UNIT")} ${index + 1}`, value: "" },
                          ...
                          { title: "PT_FORM2_BUILT_AREA", value: Math.round(Number(unit?.constructionDetail?.builtUpArea) * 9).toFixed(2) },
                        ];
              ...
              // Owner Details — renders via PropertyOwners component (name, gender, mobile, category, etc.)
