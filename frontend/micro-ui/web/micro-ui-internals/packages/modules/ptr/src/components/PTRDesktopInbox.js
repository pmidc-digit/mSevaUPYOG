...
const PTRDesktopInbox = ({ tableConfig, filterComponent, ...props }) => {
  ...
  //for now as inbox api is not showing applications(temporary solution)
  const { isLoading, isError, error, data: allApplicationsData } = Digit.Hooks.ptr.usePTRSearch({ filters: {} });
  console.log("allApplicationsData", allApplicationsData);
  const { PetRegistrationApplications: applicationsList } = allApplicationsData || {};
...
const transformed = transformApiData(applicationsList);
console.log("transformed",transformed);
  ...
  if (props.isLoading) {
    ...
    result = null;
  } else if (!transformed || transformed?.length === 0 || (useNewInboxAPI && transformed?.[0]?.dataEmpty)) {
    result =
      (EmptyInboxComp && <EmptyInboxComp data={data} />) ||
      (transformed?.length === 0 || (useNewInboxAPI && transformed?.[0]?.dataEmpty) ? (
        <Card style={{ marginTop: 20 }}>
          {t("CS_MYAPPLICATIONS_NO_APPLICATION")
            .split("\\n")
            .map((text, index) => (
