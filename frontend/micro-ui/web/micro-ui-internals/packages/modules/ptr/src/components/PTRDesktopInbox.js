...
const PTRDesktopInbox = ({ tableConfig, filterComponent, ...props }) => {
  ...
  if (props.isLoading) {
    ...
    <Card className="ptr-style-a26bda7d11">
          {t("CS_MYAPPLICATIONS_NO_APPLICATION").
      split("\\n").
      map((text, index) =>
      <p key={index} className="ptr-style-91a87015f4">
                {text}
              </p>
      )}
        </Card> :
  ...
  return (
    <div className="inbox-container">
      ...
      <div className="ptr-style-97445a8d93">
        <SearchApplication
          defaultSearchParams={props.defaultSearchParams}
          onSearch={(d) => {
            props.onSearch(d);
        ...
        <div className={`result ptr-desktop-inbox__result${!props?.isSearch ? " ptr-desktop-inbox__result--inbox" : ""}`}>
          {result}
        </div>
      </div>
    </div>);
