...
const SearchApplication = ({ onSearch, type, onClose, searchFields, searchParams, isInboxPage, defaultSearchParams, clearSearch: _clearSearch }) => {
  ...
  const clearAll = (mobileView) => {
    const mobileViewStyles = mobileView ? "ptr-mobile-margin-0" : "";
    return (
      <LinkLabel className={`ptr-inbox-search-link-label ${mobileViewStyles}`} onClick={clearSearch}>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmitInput)}>
      <React.Fragment>
        <div className={`search-container ptr-inbox-search-container ${isInboxPage ? "ptr-inbox-page-margin" : "ptr-inbox-search-revert-margin"}`}>
          <div className="search-complaint-container">
            ...
            <div className={"complaint-input-container for-pt ptr-inbox-search-input-container " + (!isInboxPage ? "for-search" : "")}>
              {searchFields
                ?.filter((e) => true)
                ?.map((input, index) => (
                  <div key={input.name} className="input-fields">
                    ...
                    {formState?.dirtyFields?.[input.name] ? (
                      <span className="inbox-search-form-error ptr-inbox-search-error">{formState?.errors?.[input.name]?.message}</span>
                    ) : null}
                  </div>
                ))}

              {isInboxPage && (
                <div className="input-fields ptr-inbox-clear-search-wrapper">
                  <div>{clearAll()}</div>
                </div>
              )}

              {type === "desktop" && !mobileView && (
                <div className="search-submit-wrapper ptr-inbox-search-submit-wrapper">
                  <SubmitBar
        ...
        {(type === "mobile" || mobileView) && (
          <ActionBar className="clear-search-container">
            <button className="clear-search ptr-inbox-mobile-clear-btn">{clearAll(mobileView)}</button>
            <SubmitBar
              disabled={!!Object.keys(formState.errors).length}
              label={t("ES_COMMON_SEARCH")}
              className="ptr-inbox-mobile-submit-btn"
              submit={true}
            />
          </ActionBar>
        )}
