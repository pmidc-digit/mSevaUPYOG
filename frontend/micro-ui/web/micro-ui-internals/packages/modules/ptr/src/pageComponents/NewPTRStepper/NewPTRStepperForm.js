...
const NewPTRStepperForm = () => {
  ...
  const tenantIdCheck = location.state?.tenantId;

  console.log("Application Number:", applicationNumber);
  console.log("Tenant ID:", tenantIdCheck);

  const shouldEnableSearch = Boolean(applicationNumber);
  ...
  useEffect(() => {
    if (applicationNumber && applicationData?.PetRegistrationApplications?.length) {
      dispatch(UPDATE_PTRNewApplication_FORM("responseData", applicationData.PetRegistrationApplications));
    }
  }, [applicationData, applicationNumber, dispatch]);
