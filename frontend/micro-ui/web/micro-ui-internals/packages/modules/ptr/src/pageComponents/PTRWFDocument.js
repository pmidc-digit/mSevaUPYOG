...
function PTRWFDocument({ value = {}, Code, index }) {
  ...
  return (
    <div className="ptr-wf-document-container">
      <React.Fragment>
        {data?.pdfFiles && (
          <div className="ptr-wf-document-wrapper">
            {documents?.map((document, index) => {
              let documentLink = pdfDownloadLink(data.pdfFiles, document?.fileStoreId);
              return (
                <a target="_" href={documentLink} className="ptr-wf-document-link" key={index}>
                  <PDFSvg width={85} height={100} className="ptr-wf-document-icon" />
