...
function PTRWFDocument({ value = {}, Code, index }) {
  ...
  return (
    <div className="ptr-style-c1186b1ba1">
      <React.Fragment>
        {data?.pdfFiles &&
        <div className="ptr-style-2fb62148d1">
            {documents?.map((document, index) => {
            let documentLink = pdfDownloadLink(data.pdfFiles, document?.fileStoreId);
            return (
              <a target="_" href={documentLink} key={index} className="ptr-style-852cda79b9">
                  <PDFSvg width={85} height={100} className="ptr-style-b54e408c7b" />
