...
export const convertEpochToDate = (dateEpoch, businessService) => {
  ...
  if (dateEpoch) {
    ...
    day = (day > 9 ? "" : "0") + day;
    if (businessService == "ptr") return `${day}-${month}-${year}`;
...
export const mapPTRApplicationDataToDefaultValues = (applicationData, t) => {
  const petDetails = applicationData?.petDetails || {};
  ...
  return {
    petDetails: {
      ...
      doctorName: petDetails?.doctorName || "",
      lastVaccineDate: petDetails?.lastVaccineDate ? convertEpochToDate(Number(petDetails?.lastVaccineDate)) : "",
      vaccinationNumber: petDetails?.vaccinationNumber || "",
      petColor: petDetails?.petColor || "",
      adoptionDate: petDetails?.adoptionDate ? convertEpochToDate(petDetails?.adoptionDate) : "",
      birthDate: petDetails?.birthDate ? convertEpochToDate(petDetails?.birthDate) : "",
      identificationMark: petDetails?.identificationMark || "",
    },
    ownerDetails: {
      firstName: applicationData?.applicantName?.split(" ")?.[0] || "",
      lastName: applicationData?.applicantName?.split(" ")?.slice(1)?.join(" ") || "",
      fatherOrHusbandName: applicationData?.fatherName || "",
      mobileNumber: applicationData?.mobileNumber || "",
      emailId: applicationData?.emailId || "",
      address: applicationData?.address?.addressId || applicationData?.address?.detail || "",
    ...
    ApplicationMeta: {
      ...
      applicationType: applicationData?.applicationType || "NEWAPPLICATION",
      validityDate: applicationData?.validityDate || "",
      status: applicationData?.status || "",
      expireFlag: applicationData?.expireFlag || false,
    },
