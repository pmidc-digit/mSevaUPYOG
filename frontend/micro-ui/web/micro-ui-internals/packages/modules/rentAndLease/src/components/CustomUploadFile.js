...
const CustomUploadFile = (props) => {
  ...
  return (
    <div>
            ...
            {props?.uploadedFiles?.map((file, index) => {
              ...
              return (
                <div className="upload-tag-container" key={index}>
                  <RemoveableTag text={file[0]} onClick={(e) => props?.removeTargetedFile(fileDetailsData, e)} />
                </div>
              );
            })}
      ...
      {props?.disabled && (
        <div className={`upload-file upload-file-min-height ${user_type === "employee" ? "" : "upload-file-max-width"}`}>
          {props.uploadedFile ? (
            <SubmitBar onSubmit={() => routeTo(props.uploadedFile)} label={t("CS_VIEW_DOCUMENT")} />
          ) : (
            <h2 className="file-upload-status">{t("ES_NO_FILE_SELECTED_LABEL")}</h2>
