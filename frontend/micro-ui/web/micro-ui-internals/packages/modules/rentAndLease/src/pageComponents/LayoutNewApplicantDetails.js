...
const formatOwnerAddress = (address) => {
  ...
  return [address.addressLine1, address.addressLine2, address.landmark, address.street, address.city, address.pincode].filter(Boolean).join(", ");
};
...
const LayoutNewApplicantDetails = (_props) => {
  ...
  const { t, currentStepData, Controller, control, setValue, errors, errorStyle, clearErrors, goNext, onGoBack, config } = _props;
  ...
  const [getUpdatedOwners, setUpdatedOwners] = useState([]);
  ...
  // Restore owners from Redux first, then fall back to the edit API response.
  useEffect(() => {
    ...
    if (ownerSource.length > 0) {
      restoredOwners = ownerSource.map((a) => ({
        ...a,
        name: a.name || "",
        mobileNumber: a.mobileNumber || a.mobileNo || "",
        emailId: a.emailId || "",
        fatherOrHusbandName: a.fatherOrHusbandName || "",
        permanentAddress: formatOwnerAddress(a.address || a.permanentAddress),
        dob: a.dob || "",
        gender: a.gender || "",
        ...
        uuid: a.uuid ?? a.userUuid ?? a.ownerId ?? null,
        status: a.status !== undefined ? a.status : a.active !== undefined ? a.active : true,
      }));
    ...
    const rawType = formattedData?.aplicantType || ownerSource?.[0]?.aplicantType;
  ...
  useEffect(() => {
    ...
    setUpdatedOwners(applicantsArray);

    dispatch(UPDATE_RENTANDLEASE_NEW_APPLICATION_FORM("applicants", applicantsArray));
    ...
    if (selectedType?.code === "INDIVIDUAL") {
      ...
      if (active.length > 1) {
        ...
        const updated = selectedOwners.map((o) =>
          o === firstActive ? { ...o, isPrimaryOwner: true } : { ...o, status: false, isPrimaryOwner: null }
        );
  ...
  const onSubmit = async () => {
    ...
    // additionalDetails["alternateMobileNumber"] = data?.[0]?.alternateMobileNumber;
  ...
  return (
    <React.Fragment>
      <div>
        ...
        {activeOwners.length > 0 && (
          <div className="obps-page-components-layout-new-applicant-details--style-7">
            ...
            <StatusTable
              className="ral-selected-owners-table"
            ...
            >
              <div style={{ width: "100%", maxWidth: "100%", overflowX: "auto", overflowY: "hidden" }}>
                <Table
                  className="ral-selected-owners-grid"
                  t={t}
                  data={activeOwners}
                  columns={tableColumns}
                  styles={{ width: "2200px", minWidth: "2200px", tableLayout: "fixed" }}
                  getCellProps={(cellInfo) => ({
                    style: {
                      padding: "12px 16px",
                      textAlign: cellInfo.column.id === "actions" ? "center" : "left",
        ...
        {activeOwners.length === 0 && (
          <CardLabelError className="obps-page-components-layout-new-applicant-details--style-9">{t("AT LEAST ONE OWNER REQUIRED")}</CardLabelError>
        )}
        ...
        {showModal && (
          <LayoutOwnerSearchModal
            ...
            isPrimaryOwner={
              editingOwner
                ? editingOwner.isPrimaryOwner === true || editingOwner.isPrimaryOwner === "true"
                : activeOwners.length === 0 || ownerType?.code === "INDIVIDUAL"
            }
          />
        )}
      </div>
      ...
      {/* Toast Notification */}
      {showToast && (
        <Toast error={showToast?.error} warning={showToast?.warning} label={t(showToast?.message)} onClose={closeToast} isDleteBtn={true} />
      )}
    </React.Fragment>
  );
};
