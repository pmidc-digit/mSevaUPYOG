...
import { CardLabel, UploadFile, Toast, FormStep, LabelFieldPair } from "@mseva/digit-ui-react-components";
...
const RentAndLeaseSelectProofIdentity = ({ t, config, onSelect, userType, formData }) => {
  ...
  const validateFile = (file, docCode) => {
    ...
    // Restrict to images only for Applicant Photo
    if (docCode === "APPLICANT.APPLICANTPHOTO") {
  ...
  const makeDocumentsValidator = (mdms) => {
    ...
    return (documents = []) => {
      ...
      for (const doc of requiredDocs) {
        ...
        if (!satisfied) {
          missingDocs.push(doc.name || t(doc.code.replace(/\./g, "_")));
        }
      }
  ...
  // Adapt MDMS hook for RentAndLease documents
  // const { data: mdmsDocsData, isLoading } = Digit.Hooks.rentAndLease?.useDocumentsMDMS(tenantId);
  const mdmsDocsData = [];
  ...
  useEffect(() => {
    if (Array.isArray(apiDataCheck) && apiDataCheck.length > 0 && Array.isArray(apiDataCheck[0].documents)) {
      const docs = apiDataCheck[0].documents.map((d) => {
        const fileId = d?.fileStoreId ?? d?.filestoreId ?? d?.documentUid ?? null;
  ...
  // Remove auto-send, let FormStep handle submission
  // useEffect(() => {
  //   if (!_.isEqual(lastSentRef.current, documents) && onSelect) {
  ...
  const handleSubmit = () => {
    if (Object.keys(formErrors).length > 0) {
      setToastError(t(formErrors.missingRequired || "PTR_MISSING_REQUIRED_DOCUMENTS"));
      onSelect(config.key, { missingDocs: formErrors.missingDocs || [] });
      return;
    }
    let documentStep = { documents };
...
function RentAndLeaseSelectDocument({ t, document: doc, setDocuments, documents, validateFile, makeDocumentsValidator, mdms, setFormErrors }) {
  ...
  const handleRentAndLeaseSelectDocument = (value) => setSelectedDocument(value);
  ...
  useEffect(() => {
    if (file) {
      (async () => {
        setLoading(true);
        try {
          const response = await Digit.UploadServices.Filestorage("RENTANDLEASE", file, Digit.ULBService.getStateId());
          setLoading(false);
  ...
  return (
    <div style={{ marginBottom: "24px" }}>
      ...
      <LabelFieldPair>
        <CardLabel className="card-label-smaller">{t(doc?.code.replace(/\./g, "_")) + (doc?.required ? "  *" : "")}</CardLabel>
      </LabelFieldPair>
      ...
      <LabelFieldPair>
        ...
        <div className="field">
          <UploadFile
            ...
            accept={doc?.code === "APPLICANT.APPLICANTPHOTO" ? ".jpeg, .jpg, .png" : ".pdf, .jpeg, .jpg, .png"}
            buttonType="button"
