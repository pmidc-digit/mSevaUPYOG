import { CardLabel, FormStep, RadioButtons, TextInput, CheckBox, LinkButton, MobileNumber, Toast, Dropdown } from "@mseva/digit-ui-react-components";
...
const SVApplicantDetails = ({ t, config, onSelect, userType, formData, editdata, previousData }) => {
  ...
  const [fatherName, setfatherName] = useState(
    editdata?.vendorDetail?.[0]?.fatherName || formData?.owner?.units?.fatherName || formData?.owner?.fatherName || ""
  );
  ...
  const [dependentDateBirth, setdependentDateBirth] = useState(
    formData?.owner?.units?.dependentDateBirth || formData?.owner?.dependentDateBirth || ""
  );
    ...
    (formData?.owner && formData?.owner?.units) || [
      {
        ...
        mobileNumber:
          previousData?.vendorDetail?.[0]?.mobileNo ||
          editdata?.vendorDetail?.[0]?.mobileNo ||
          (user?.type === "CITIZEN" ? user?.mobileNumber : "") ||
          "",
        ...
        email:
          previousData?.vendorDetail?.[0]?.emailId || editdata?.vendorDetail?.[0]?.emailId || (user?.type === "CITIZEN" ? user?.emailId : "") || "",
  ...
  const handleSaveasDraft = () => {
    ...
    if (!isEmpty(fields?.[0]?.vendorName)) {
      ...
      if (isEmpty(spouseName) && isEmpty(dependentName)) {
        ...
        vendordetails = [createVendorObject(fields), createSpouseObject(fields)];
      } else if (!isEmpty(spouseName) && !isEmpty(dependentName)) {
        // Case 3: All three exist (vendor, spouse, and dependent)
        vendordetails = [createVendorObject(fields), createSpouseObject(fields), createDependentObject(fields)];
      }
    }
  ...
  return (
    <React.Fragment>
      ...
      <FormStep
        ...
        isDisabled={
          !fields[0].vendorName ||
          !fields[0].userCategory ||
          !fields[0].vendorDateOfBirth ||
          !fields[0].gender ||
          !fields[0].fatherName ||
          (spouseDependentChecked && !fields[0].spouseName) ||
          (dependentNameChecked && !fields[0].dependentName)
        }
      >
        {fields.map((field, index) => {
          return (
            <div key={`${field}-${index}`}>
              ...
              >
                <CardLabel>
                  {`${t("SV_VENDOR_NAME")}`} <span className="astericColor">*</span>
                </CardLabel>
                <LinkButton
                  label={
                    <div>
                ...
                />
                <CardLabel>
                  {`${t("SV_OWNER_CATEGORY")}`} <span className="astericColor">*</span>
                </CardLabel>
                <Controller
                  control={control}
                ...
                <CardLabel>
                  {`${t("SV_REGISTERED_MOB_NUMBER")}`} <span className="astericColor">*</span>
                </CardLabel>
                <MobileNumber
                  style={{ background: "#FAFAFA" }}
                ...
                <CardLabel>
                  {`${t("SV_DATE_OF_BIRTH")}`} <span className="astericColor">*</span>
                </CardLabel>
                <TextInput
                  style={{ background: "#FAFAFA" }}
                  t={t}
                ...
                />
                <CardLabel>
                  {`${t("SV_GENDER")}`} <span className="astericColor">*</span>
                </CardLabel>
                <RadioButtons
                  t={t}
                  options={common}
                ...
                />
                <CardLabel>
                  {`${t("SV_FATHER_NAME")}`} <span className="astericColor">*</span>
                </CardLabel>
                <TextInput
                  style={{ background: "#FAFAFA" }}
                  t={t}
              ...
              >
                ...
                <div>
                  <div style={{ display: "flex", gap: "22px" }}>
                    <CardLabel>
                      {`${t("SV_SPOUSE_NAME")}`} {spouseDependentChecked && <span className="astericColor">*</span>}
                    </CardLabel>
                    <CheckBox label={t("SV_INVOLVED_IN_VENDING")} onChange={setSpouseDependentHandler} checked={spouseDependentChecked} />
                  </div>
                  <TextInput
                    style={{ background: "#FAFAFA" }}
                    t={t}
                ...
                <CardLabel>
                  {`${t("SV_SPOUSE_DATE_OF_BIRTH")}`} {spouseDependentChecked && <span className="astericColor">*</span>}
                </CardLabel>
                <TextInput
                  style={{ background: "#FAFAFA" }}
                  t={t}
              ...
              >
                <div>
                  <div style={{ display: "flex", gap: "22px" }}>
                    <CardLabel>
                      {`${t("SV_DEPENDENT_NAME")}`} {dependentNameChecked && <span className="astericColor">*</span>}
                    </CardLabel>
                    <CheckBox label={t("SV_INVOLVED_IN_VENDING")} onChange={setDependentNameHandler} checked={dependentNameChecked} />
                  </div>
                  <TextInput
                    style={{ background: "#FAFAFA" }}
                    t={t}
                ...
                <CardLabel>
                  {`${t("SV_DEPENDENT_DATE_OF_BIRTH")}`} {dependentNameChecked && <span className="astericColor">*</span>}
                </CardLabel>
                <TextInput
                  style={{ background: "#FAFAFA" }}
                  t={t}
                ...
                />
                <CardLabel>
                  {`${t("SV_DEPENDENT_GENDER")}`} {dependentNameChecked && <span className="astericColor">*</span>}
                </CardLabel>
                <RadioButtons
                  t={t}
                  options={common}
