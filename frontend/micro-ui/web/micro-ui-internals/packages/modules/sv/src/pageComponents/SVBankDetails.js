...
import { FormStep, TextInput, CardLabel, Toast } from "@mseva/digit-ui-react-components";
...
const SVBankDetails = ({ t, config, onSelect, userType, formData, editdata, previousData }) => {
  const [bankDetails, setBankDetails] = useState({
    ...
    confirmAccountNumber:
      formData?.bankdetails?.confirmAccountNumber || previousData?.bankDetail?.accountNumber || editdata?.bankDetail?.accountNumber || "",
    ...
    accountHolderName:
      formData?.bankdetails?.accountHolderName || previousData?.bankDetail?.accountHolderName || editdata?.bankDetail?.accountHolderName || "",
  });
  ...
  return (
    <React.Fragment>
      ...
      <FormStep config={config} onSelect={goNext} onSkip={() => onSelect()} t={t}>
        {[
