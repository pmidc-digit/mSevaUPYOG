...
import { FormStep, TextInput, CardLabel, Dropdown, LinkButton, Toast } from "@mseva/digit-ui-react-components";
...
const SVBusinessDetails = ({ t, config, onSelect, userType, formData, editdata, previousData }) => {
  ...
  const [vendingType, setvendingType] = useState(
    formData?.businessDetails?.vendingType ||
      convertToObject(previousData?.vendingActivity || editdata?.vendingActivity) ||
      formData?.businessDetails?.vendingType ||
      ""
  );
  const [vendingZones, setvendingZones] = useState(
    formData?.businessDetails?.vendingZones || convertToObject(previousData?.vendingZone || editdata?.vendingZone) || ""
  );
  const [location, setlocation] = useState(formData?.businessDetails?.location || "");
  const [areaRequired, setareaRequired] = useState(
    formData?.businessDetails?.areaRequired || previousData?.vendingArea || editdata?.vendingArea || ""
  );
  const [nameOfAuthority, setnameOfAuthority] = useState(
    formData?.businessDetails?.nameOfAuthority || previousData?.localAuthorityName || editdata?.localAuthorityName || ""
  );
  const [vendingLiscence, setvendingLiscence] = useState(
    formData?.businessDetails?.vendingLiscence || previousData?.vendingLiscence || editdata?.vendingLiscence || ""
  );
  const inputStyles = { width: user.type === "EMPLOYEE" ? "50%" : "86%" };
  const [showToast, setShowToast] = useState(null);
  const [isSameForAll, setIsSameForAll] = useState(
    previousData?.vendingOperationTimeDetails?.length === 7 || editdata?.vendingOperationTimeDetails?.length === 7
      ? true
      : false || formData?.businessDetails?.isSameForAll
  ); // Flag to check if same for all days
  const [daysOfOperation, setDaysOfOperation] = useState(
    // Array to store selected days of operation
    formData?.businessDetails?.daysOfOperation || [
      {
        name: "Monday",
        isSelected: false,
        startTime: previousData?.vendingOperationTimeDetails?.[0]?.fromTime || editdata?.vendingOperationTimeDetails?.[0]?.fromTime || "",
        endTime: previousData?.vendingOperationTimeDetails?.[0]?.toTime || editdata?.vendingOperationTimeDetails?.[0]?.toTime || "",
      },
      {
        name: "Tuesday",
        isSelected: false,
        startTime: previousData?.vendingOperationTimeDetails?.[1]?.fromTime || editdata?.vendingOperationTimeDetails?.[1]?.fromTime || "",
        endTime: previousData?.vendingOperationTimeDetails?.[1]?.toTime || editdata?.vendingOperationTimeDetails?.[1]?.toTime || "",
      },
      {
        name: "Wednesday",
        isSelected: false,
        startTime: previousData?.vendingOperationTimeDetails?.[2]?.fromTime || editdata?.vendingOperationTimeDetails?.[2]?.fromTime || "",
        endTime: previousData?.vendingOperationTimeDetails?.[2]?.toTime || editdata?.vendingOperationTimeDetails?.[2]?.toTime || "",
      },
      {
        name: "Thursday",
        isSelected: false,
        startTime: previousData?.vendingOperationTimeDetails?.[3]?.fromTime || editdata?.vendingOperationTimeDetails?.[3]?.fromTime || "",
        endTime: previousData?.vendingOperationTimeDetails?.[3]?.toTime || editdata?.vendingOperationTimeDetails?.[3]?.toTime || "",
      },
      {
        name: "Friday",
        isSelected: false,
        startTime: previousData?.vendingOperationTimeDetails?.[4]?.fromTime || editdata?.vendingOperationTimeDetails?.[4]?.fromTime || "",
        endTime: previousData?.vendingOperationTimeDetails?.[4]?.toTime || editdata?.vendingOperationTimeDetails?.[4]?.toTime || "",
      },
      {
        name: "Saturday",
        isSelected: false,
        startTime: previousData?.vendingOperationTimeDetails?.[5]?.fromTime || editdata?.vendingOperationTimeDetails?.[5]?.fromTime || "",
        endTime: previousData?.vendingOperationTimeDetails?.[5]?.toTime || editdata?.vendingOperationTimeDetails?.[5]?.toTime || "",
      },
      {
        name: "Sunday",
        isSelected: false,
        startTime: previousData?.vendingOperationTimeDetails?.[6]?.fromTime || editdata?.vendingOperationTimeDetails?.[6]?.fromTime || "",
        endTime: previousData?.vendingOperationTimeDetails?.[6]?.toTime || editdata?.vendingOperationTimeDetails?.[6]?.toTime || "",
      },
    ]
  );
  ...
  const validateDaysOfOperation = () => {
    const atLeastOneDaySelected = daysOfOperation.some(
      (day) => day.startTime,
      (day) => day.endTime
    );
  ...
  const handleToggleIsSameForAll = () => {
    if (!isSameForAll) {
      ...
      const { startTime, endTime, isSelected } = daysOfOperation.find((day) => day.startTime || day.endTime) || daysOfOperation[0];
      ...
      <div>
        ...
        {!isOpen && (
          <FormStep
            ...
            t={t}
            isDisabled={
              !vendingType || !vendingZones || (vendingType?.code === "STATIONARY" ? !areaRequired : null) || !nameOfAuthority || !daysOfOperation
            }
          >
            <div>
              <CardLabel>
                {`${t("SV_VENDING_TYPE")}`} <span className="astericColor">*</span>
              </CardLabel>
              <Controller
                control={control}
                name={"vendingType"}
              ...
              />
              <CardLabel>
                {`${t("SV_VENDING_ZONES")}`} <span className="astericColor">*</span>
              </CardLabel>
              <Controller
                control={control}
                name={"vendingZones"}
                ...
                <LinkButton
                  label={
                    <div>
                      <span>
                        <svg
                          style={{ float: "right", position: "relative", bottom: "25px", marginTop: "-17px", marginRight: "51%" }}
                          width="24"
                          height="24"
                          viewBox="0 0 24 24"
                          fill="none"
              ...
              <CardLabel>
                {t("SV_DAY_HOUR_OPERATION")} <span className="astericColor">*</span>
              </CardLabel>
              <ApplicationTable
                t={t}
              ...
              {vendingType?.code === "STATIONARY" && (
                <React.Fragment>
                  <CardLabel>
                    {`${t("SV_AREA_REQUIRED")}`} <span className="astericColor">*</span>
                  </CardLabel>
                  <TextInput
                    t={t}
                    type={"text"}
              ...
              <CardLabel>
                {`${t("SV_LOCAL_AUTHORITY_NAME")}`} <span className="astericColor">*</span>
              </CardLabel>
              <TextInput
                t={t}
                type={"text"}
