...
*/
import { Card, CardHeader, CardSubHeader, CheckBox, LinkButton, Row, StatusTable, SubmitBar, EditIcon } from "@mseva/digit-ui-react-components";
...
const SVCheckPage = ({ onSubmit, editdata, value = {} }) => {
  ...
  const {
    ...
    error,
  } = Digit.Hooks.useDocumentSearch(improvedDoc, { enabled: improvedDoc?.length > 0 ? true : false });
  let applicationDocs = [];
      ...
      <Card>
        ...
        <div>
          ...
          <StatusTable style={{ marginTop: "30px", marginBottom: "30px" }}>
            ...
            <Row label={t("SV_REGISTERED_MOB_NUMBER")} text={`${t(checkForNA(owner?.units?.[0]?.mobileNumber))}`} />

            <Row label={t("SV_DATE_OF_BIRTH")} text={`${t(checkForNA(owner?.units?.[0]?.vendorDateOfBirth))}`} />

            <Row label={t("SV_GENDER")} text={`${t(checkForNA(owner?.units?.[0]?.gender?.code))}`} />
            <Row label={t("SV_FATHER_NAME")} text={`${t(checkForNA(owner?.units?.[0]?.fatherName))}`} />
            {owner?.units?.[0]?.email ? <Row label={t("SV_EMAIL")} text={`${t(checkForNA(owner?.units?.[0]?.email))}`} /> : null}
              ...
              <Row
                ...
                text={
                  <span>
                    ...
                    <span className="astericColor">({t(owner?.spouseDependentChecked ? "INVOLVED" : "NOT INVOLVED")})</span>
                  </span>
                }
              />
            )}
            {owner?.units?.[0]?.spouseDateBirth ? (
              <Row label={t("SV_SPOUSE_DATE_OF_BIRTH")} text={`${t(checkForNA(owner?.units?.[0]?.spouseDateBirth))}`} />
            ) : null}
              ...
              <Row
                ...
                text={
                  <span>
                    ...
                    <span className="astericColor">({t(owner?.dependentNameChecked ? "INVOLVED" : "NOT INVOLVED")})</span>
                  </span>
                }
              />
            )}
            {owner?.units?.[0]?.dependentDateBirth ? (
              <Row label={t("SV_DEPENDENT_DATE_OF_BIRTH")} text={`${t(checkForNA(owner?.units?.[0]?.dependentDateBirth))}`} />
            ) : null}
            {owner?.units?.[0]?.dependentGender ? (
              <Row label={t("SV_DEPENDENT_GENDER")} text={`${t(checkForNA(owner?.units?.[0]?.dependentGender?.code))}`} />
            ) : null}
            {owner?.units?.[0]?.dependentGender ? (
              <Row label={t("SV_TRADE_NUMBER")} text={`${t(checkForNA(owner?.units?.[0]?.tradeNumber))}`} />
            ) : null}
          </StatusTable>
          ...
          <StatusTable style={{ marginTop: "30px", marginBottom: "30px" }}>
            ...
            />
            <Row label={t("SV_VENDING_ZONES")} text={`${t(checkForNA(businessDetails?.vendingZones?.value))}`} />
            <Row label={t("SV_AREA_REQUIRED")} text={`${t(checkForNA(businessDetails?.areaRequired))}`} />
            <Row label={t("SV_LOCAL_AUTHORITY_NAME")} text={`${t(checkForNA(businessDetails?.nameOfAuthority))}`} />
            {businessDetails?.vendingLiscence ? (
              <Row label={t("SV_VENDING_LISCENCE")} text={`${t(checkForNA(businessDetails?.vendingLiscence))}`} />
            ) : null}
            ...
            bankdetails?.accountHolderName && (
              <React.Fragment>
                ...
                <StatusTable style={{ marginTop: "30px", marginBottom: "30px" }}>
                  ...
                  />
                  <Row label={t("SV_IFSC_CODE")} text={`${t(checkForNA(bankdetails?.ifscCode))}`} />
                  <Row label={t("SV_BANK_NAME")} text={`${t(checkForNA(bankdetails?.bankName))}`} />
                  <Row label={t("SV_BANK_BRANCH_NAME")} text={`${t(checkForNA(bankdetails?.bankBranchName))}`} />
                  <Row label={t("SV_ACCOUNT_HOLDER_NAME")} text={`${t(checkForNA(bankdetails?.accountHolderName))}`} />
                </StatusTable>
              </React.Fragment>
            )}
          ...
          <StatusTable style={{ marginTop: "30px", marginBottom: "30px" }}>
            ...
            />
            <Row label={t("SV_ADDRESS_LINE2")} text={`${t(checkForNA(address?.addressline2))}`} />
            <Row label={t("SV_CITY")} text={`${t(checkForNA(address?.city?.city?.name))}`} />
            <Row label={t("SV_LOCALITY")} text={`${t(checkForNA(address?.locality?.i18nKey))}`} />
            {address?.pincode ? <Row label={t("SV_ADDRESS_PINCODE")} text={`${t(checkForNA(address?.pincode))}`} /> : null}
            {address?.landmark ? <Row label={t("SV_LANDMARK")} text={`${t(checkForNA(address?.landmark))}`} /> : null}
          </StatusTable>
          ...
          <StatusTable style={{ marginBottom: "30px" }}>
            ...
            {specialCategoryData?.beneficiary !== null ? (
              <Row label={t("SV_BENEFICIARY_SCHEMES")} text={`${t(checkForNA(specialCategoryData?.beneficiary?.value))}`} />
            ) : null}
            {specialCategoryData?.enrollmentId !== null ? (
              <Row label={t("SV_ENROLLMENT_APPLICATION_NUMBER")} text={`${t(checkForNA(specialCategoryData?.enrollmentId))}`} />
            ) : null}
          </StatusTable>
