import { Toast, Card, KeyNote, SubmitBar } from "@mseva/digit-ui-react-components";
...
const StreetVendingApplication = ({ application, buttonLabel, previousDraftId, onDiscard }) => {
  ...
  return (
    <Card>
      ...
      {application?.vendingActivity === "STATIONARY" && <KeyNote keyValue={t("SV_AREA_REQUIRED")} note={application?.vendingArea} />}
      ...
      <div style={{ display: "flex", gap: "5px" }}>
        <Link
          to={isDraft ? `/digit-ui/citizen/sv/apply/info` : `/digit-ui/citizen/sv/application/${application?.applicationNo}/${application?.tenantId}`}
        >
