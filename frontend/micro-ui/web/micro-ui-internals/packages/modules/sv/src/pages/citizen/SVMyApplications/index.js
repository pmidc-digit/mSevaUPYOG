import { Header, Loader, TextInput, Dropdown, SubmitBar, CardLabel, Card, MobileNumber } from "@mseva/digit-ui-react-components";
...
export const SVMyApplications = () => {
  ...
  useEffect(() => {
    if (draftData?.SVDetail && nonDraftData?.SVDetail) {
      setApplicationsList([...(draftData.SVDetail || []), ...(nonDraftData.SVDetail || [])]);
    }
  ...
  const handleDiscardApplication = (applicationToRemove) => {
    ...
    setApplicationsList((prevApplications) => prevApplications.filter((app) => app !== applicationToRemove));
  };
      ...
      <div>
        {applicationsList?.length > 0 &&
          applicationsList.map((application, index) => (
            <div key={index}>
              <StreetVendingApplication
                application={application}
                tenantId={user?.permanentCity}
                buttonLabel={t("SV_TRACK")}
                previousDraftId={previousDraftId}
                onDiscard={handleDiscardApplication}
              />
            </div>
          ))}
