import { Header, MultiLink, SubmitBar } from "@mseva/digit-ui-react-components";
...
const ApplicationDetails = () => {
    ...
    workflowDetails.data.actionState.nextActions = workflowDetails?.data?.actionState?.nextActions.map((act) => {
      if (act.action === "PAY") {
        return {
          ...
          forcedName: "SV_PAY",
          redirectionUrl: {
            pathname: `/digit-ui/employee/payment/collect/sv-services/${appDetailsToShow?.applicationData?.applicationData?.applicationNo}`,
          },
        };
      }
      return act;
  ...
  const printCertificate = async () => {
    let response = await Digit.PaymentService.generatePdf(
      tenantId,
      { SVDetail: [applicationDetails?.applicationData?.applicationData] },
      "svcertificate"
    );
    const fileStore = await Digit.PaymentService.printReciept(tenantId, { fileStoreIds: response.filestoreIds[0] });
    window.open(fileStore[response?.filestoreIds[0]], "_blank");
