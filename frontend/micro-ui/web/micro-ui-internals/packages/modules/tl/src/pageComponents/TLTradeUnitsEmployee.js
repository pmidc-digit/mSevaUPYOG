...
const TradeUnitForm = (_props) => {
    ...
    return sortDropdownNames(
      tradeSubTypeOptionsList.filter((subType) => {
        return billingSlabTradeTypeData?.some(
          (ob) => ob?.tradeType === subType?.code && ob?.structureType === formData?.tradedetils?.[0]?.structureSubType?.code
        );
      }),
      "i18nKey",
      t
  ...
  useEffect(() => {
    if (
      tradeTypeMdmsData?.length > 0 &&
      (ckeckingLocation || (unit?.tradeCategory && tradeTypeOptionsList && tradeTypeOptionsList?.length == 0)) &&
      !isLoading &&
      !isbillingSlabLoading
    ) {
  ...
  useEffect(() => {
    if (
      tradeTypeMdmsData?.length > 0 &&
      (ckeckingLocation || (unit?.tradeType && tradeSubTypeOptionsList && tradeSubTypeOptionsList?.length == 0)) &&
      !isLoading &&
      !isbillingSlabLoading
    ) {
      let tradeType = cloneDeep(tradeTypeMdmsData);
  ...
  function checkRangeForUomValue(e, fromUom, toUom) {
    let selectedtradesubType = billingSlabTradeTypeData?.filter(
      (ob) => ob?.tradeType === unit?.tradeSubType?.code && ob?.structureType === formData?.tradedetils?.[0]?.structureSubType?.code
    )?.[0];
  ...
  function getUomRange(type) {
    ...
    if (type === "fromUom") return selectedtradesubType?.fromUom;
    else return selectedtradesubType?.toUom;
  }
  ...
  function checkBillingSlab(value) {
    if (
      ...
      (billingSlabTradeTypeData?.filter(
        (ob) => ob?.tradeType === value?.code && ob?.structureType === formData?.tradedetils?.[0]?.structureSubType?.code
      )?.length <= 0 ||
        billingSlabTradeTypeData?.filter(
          (ob) => ob?.tradeType === value?.code && ob?.structureType === formData?.tradedetils?.[0]?.structureSubType?.code
        ) == undefined)
          ...
          <LabelFieldPair>
            ...
            <Controller
              ...
              rules={{
                ...
                validate: {
                  pattern: (val) =>
                    /*/^(0)*[1-9][0-9]{0,5}$/.test(val)*/ checkBillingSlab(val || unit?.tradeSubType)
                      ? true
                      : t("TL_BILLING_SLAB_NOT_FOUND_FOR_COMB"),
                },
              }}
              render={(props) => (
                <Dropdown
                      ...
                      pattern: (val) =>
                        /*/^(0)*[1-9][0-9]{0,5}$/.test(val)*/ val > 0 && val < 99999
                          ? checkRangeForUomValue(val, unit?.tradeSubType?.fromUom, unit?.tradeSubType?.toUom)
                            ? true
                            : `${t("ERR_WRONG_UOM_VALUE")} ${getUomRange("fromUom")} - ${getUomRange("toUom")}`
