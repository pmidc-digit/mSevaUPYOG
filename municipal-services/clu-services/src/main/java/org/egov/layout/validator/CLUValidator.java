...
public class CLUValidator {
	 ...
	 * @param mdmsData
	 */
	public void validateCreate(CluRequest nocRequest, Object mdmsData) {
	 ...
	 * @param mdmsData
	 */
	public void validateUpdate(CluRequest nocRequest, Clu searchResult, String mode, Object mdmsData) {
	 ...
	 * @param mdmsData
	 */
	private void validateData(Clu searchResult, Clu noc, String mode, Object mdmsData) {
	...
	public Map<String, String> getOrValidateBussinessService(Clu noc, Object mdmsData) {
		...
		String filterExp = "$.[?(@.code == '" + noc.getCluType() + "' )]";
		List<Map<String, Object>> jsonOutput = JsonPath.read(result, filterExp);
		if (jsonOutput.isEmpty()) {
			throw new CustomException("MDMS DATA ERROR", "Unable to fetch " + noc.getCluType() + " workflow mode from MDMS");
		}
...
//		Object additionalDetailsObj = layout.getNocDetails().getAdditionalDetails();
	 ...
	 * @param mdmsData
	 */
	private void validateAttachedDocumentTypes(Clu noc, Object mdmsData) {
...
////			});
	 ...
	 * @param noc
	 */
	private void validateDuplicateDocuments(Clu noc) {
					...
					if (requiredDocTypes.size() > 0 && documents.size() < requiredDocTypes.size()) {
						...
						addedDocTypes.forEach(documentType -> {
							if (!docTypes.contains(documentType)) {
								throw new CustomException("NOC_INVALID_DOCUMENTTYPE", "Document Type " + documentType + " is invalid for " + noc.getCluType() + " application");
							}
						});
					}
				});
