...
public class CommunityHallBookingRepositoryImpl implements CommunityHallBookingRepository {
	...
	public void saveCommunityHallBooking(CommunityHallBookingRequest bookingRequest) {
		...
		producer.push(bookingConfiguration.getCommunityHallBookingSaveTopic(),bookingRequest.getHallsBookingApplication().getBookingNo(), bookingRequest);
	...
	public void saveCommunityHallBookingInit(CommunityHallBookingRequest bookingRequest) {
		...
		producer.push(bookingConfiguration.getCommunityHallBookingInitSaveTopic(),bookingRequest.getHallsBookingApplication().getBookingId(), bookingPersiter);
	...
	public void updateBooking(@Valid CommunityHallBookingRequest communityHallsBookingRequest) {
		...
		producer.push(bookingConfiguration.getCommunityHallBookingUpdateTopic(), communityHallsBookingRequest.getHallsBookingApplication().getBookingNo(),communityHallsBookingRequest);
	}
	...
	public void createBookingTimer(CommunityHallSlotSearchCriteria criteria, RequestInfo requestInfo,
		...
		// Defensive checks to avoid NPEs and provide actionable errors
		if (criteria == null) {
			throw new org.egov.tracer.model.CustomException("INVALID_TIMER_REQUEST", "Slot search criteria cannot be null when creating timer");
		}
		if (requestInfo == null || requestInfo.getUserInfo() == null || requestInfo.getUserInfo().getUuid() == null) {
			throw new org.egov.tracer.model.CustomException("INVALID_REQUEST_INFO", "RequestInfo.userInfo.uuid is required to create a payment timer");
		}
		if (criteria.getBookingId() == null || criteria.getBookingId().isEmpty()) {
			throw new org.egov.tracer.model.CustomException("MISSING_BOOKING_ID", "bookingId is required to create a payment timer");
		}
		if (criteria.getTenantId() == null || criteria.getTenantId().isEmpty()) {
			throw new org.egov.tracer.model.CustomException("MISSING_TENANT", "tenantId is required to create a payment timer");
		}
		if (criteria.getCommunityHallCode() == null || criteria.getCommunityHallCode().isEmpty()) {
			throw new org.egov.tracer.model.CustomException("MISSING_COMMUNITY_HALL_CODE", "communityHallCode is required to create a payment timer");
		}
		if (criteria.getHallCode() == null || criteria.getHallCode().isEmpty()) {
			throw new org.egov.tracer.model.CustomException("MISSING_HALL_CODE", "hallCode is required to create a payment timer");
		}
		if (criteria.getBookingStartDate() == null || criteria.getBookingEndDate() == null) {
			throw new org.egov.tracer.model.CustomException("MISSING_BOOKING_DATES", "bookingStartDate and bookingEndDate are required to create a payment timer");
		}
