...
var options = {
  ...
  concurrency: envVariables.KAFKA_CONCURRENCY,
...
consumerGroup.on("message", function (message) {
  try {
    ...
    logger.error(`Error processing message from topic ${message.topic}: ${error.stack || error}`);
    producer.send({
      topic: envVariables.KAFKA_TOPICS_FIRENOC_DLQ,
      messages: [{
    ...
    }).catch(err => logger.error(`Error sending to DLQ: ${err.stack || err}`));
  }
});
