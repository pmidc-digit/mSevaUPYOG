package org.egov.gccalculation.config;
...
@Component
public class GCCalculationConfiguration {
	...
	@Value("${gc.module.code}")
	private String gcModuleCode;

	@Value("${gc.module.minpayable.amount}")
	private Integer ptMinAmountPayable;

	@Value("${gc.financialyear.start.month}")
	...
	@Value("${egov.demand.minimum.payable.amount}")
	 ...
	 @Value("${egov.gc.search.endpoint}")
	 private String waterConnectionSearchEndPoint;
	 
	 //Demand Topic
	 @Value("${gc.calculator.demand.successful.topic}")
	 private String onDemandsSaved;

	 @Value("${gc.calculator.demand.failed}")
	...
	@Value("${egov.garbagecalculatorservice.createdemand.topic}")
	private String createDemand;
	
    	@Value("${gc.demand.based.batch.size}")
	...
	@Value("${egov.garbagecalculatorservice.billgenerate.topic}")
