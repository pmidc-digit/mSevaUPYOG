package org.egov.gccalculation.repository.builder;
...
@Component
public class GCCalculatorQueryBuilder {
	...
	private static final String connectionNoListQuery = "SELECT distinct(conn.connectionno),ws.connectionexecutiondate FROM eg_gc_connection conn INNER JOIN eg_gc_service ws ON conn.id = ws.connection_id";
	...
	private static final String BILL_SCHEDULER_STATUS_UPDATE_QUERY = "UPDATE eg_gc_scheduler SET status=? where id=?";
	private static final String LEFT_OUTER_JOIN_STRING = " LEFT OUTER JOIN ";
	private static final String billGenerationSchedulerSearchQuery = "SELECT * from eg_gc_scheduler ";
	private static final String GARBAGE_SEARCH_QUERY = "SELECT conn.*, wc.*, document.*, wc.connectionCategory, wc.connectionType,"
			+ " wc.connection_id as connection_Id, wc.connectionExecutionDate, wc.appCreatedDate,"
			...
			+ " conn.createdTime as ws_createdTime, conn.lastModifiedTime as ws_lastModifiedTime,conn.additionaldetails, cconn.frequency_of_garbage_collection,"
...
//			+ LEFT_OUTER_JOIN_STRING + "eg_ws_roadcuttinginfo roadcuttingInfo ON roadcuttingInfo.wsid = conn.id ";
	...
	private static final String GARBAGE_SEARCH_CONNECTION_QUERY = "SELECT conn.*, wc.*,  wc.connectionCategory, wc.connectionType,"
			+ "wc.connection_id as connection_Id, wc.connectionExecutionDate,wc.appCreatedDate,"
			+ " wc.detailsprovidedby, wc.estimationfileStoreId , wc.sanctionfileStoreId , wc.estimationLetterDate,"
			+ " conn.id as conn_id, conn.tenantid, conn.applicationNo, conn.applicationStatus, conn.status, conn.connectionNo, conn.oldConnectionNo, conn.property_id,"
			+ " conn.action, conn.adhocpenalty, conn.adhocrebate, conn.adhocpenaltyreason, conn.applicationType, conn.dateEffectiveFrom,"
			...
			+ " conn.locality, conn.isoldapplication,"
//			+ " plumber.name as plumber_name, plumber.licenseno, roadcuttingInfo.id as roadcutting_id, roadcuttingInfo.roadtype as roadcutting_roadtype, roadcuttingInfo.roadcuttingarea as roadcutting_roadcuttingarea, roadcuttingInfo.roadcuttingarea as roadcutting_roadcuttingarea,"
//			+ " roadcuttingInfo.active as roadcutting_active, plumber.mobilenumber as plumber_mobileNumber, plumber.gender as plumber_gender, plumber.fatherorhusbandname, plumber.correspondenceaddress,"
			+ holderSelectValues + " FROM eg_gc_connection conn " + INNER_JOIN_STRING
			+ " eg_gc_service wc ON wc.connection_id = conn.id" + LEFT_OUTER_JOIN_STRING
...
//			+ LEFT_OUTER_JOIN_STRING + "eg_ws_roadcuttinginfo roadcuttingInfo ON roadcuttingInfo.wsid = conn.id ";
	...
	private static final String WATER_SEARCH_DEMAND_QUERY = "SELECT conn.*, wc.*, wc.connectionCategory, wc.connectionType"
			...
			+ " conn.adhocpenaltycomment, conn.adhocrebatereason, conn.adhocrebatecomment, conn.createdBy as ws_createdBy, conn.lastModifiedBy as ws_lastModifiedBy,"
			+ " conn.createdTime as ws_createdTime, conn.lastModifiedTime as ws_lastModifiedTime,conn.additionaldetails, "
			+ " conn.locality, conn.isoldapplication," + holderSelectValues
			+ " FROM eg_gc_connection conn " + INNER_JOIN_STRING + " eg_gc_service wc ON wc.connection_id = conn.id"
			+ LEFT_OUTER_JOIN_STRING
			+ "eg_gc_connectionholder connectionholder ON connectionholder.connectionid = conn.id";
	public static final String EG_GC_BILL_SCHEDULER_CONNECTION_STATUS_INSERT = "INSERT INTO eg_gc_bill_scheduler_connection_status "
			+ "(id, eg_gc_scheduler_id, locality, module, createdtime, lastupdatedtime, status, tenantid, reason, consumercode) "
			+ "VALUES (?,?,?,?,?,?,?,?,?,?);";

	public static final String RELATED_SW_CONNECTION_SEARCH_QUERY = "SELECT conn.relatedSwConn from eg_gc_connection conn ";
	...
	public String getConnectionNumberList(String tenantId, String connectionType, String status, Long taxPeriodFrom,
		...
		preparedStatement.add(GCCalculationConstant.CONNECTION_ACTIVATED);
		preparedStatement.add(GCCalculationConstant.MODIFIED_APPROVED);
...
//		query.append(" conn.connectionno = '0603000900' ");
	...
	public String fetchConnectionsToBeGenerate(String tenantId, Long taxPeriodFrom, Long taxPeriodTo,
		...
		preparedStatement.add(GCCalculationConstant.SERVICE_FIELD_VALUE_WS);
	...
	public String getConnectionNumberList(String tenantId, String connectionType, List<Object> preparedStatement,
		...
		addClauseIfRequired(preparedStatement, query);
		query.append(
				" conn.connectionno NOT IN (select distinct(consumercode) from egbs_demand_v1 dmd where (dmd.taxperiodfrom >= ? and dmd.taxperiodto <= ?) and businessservice = 'GC' and tenantid=?)");
	...
	public String getConnectionNumber(String tenantId, String consumerCode, String connectionType,
		...
		// StringBuilder query = new StringBuilder(connectionNoListQuery);
		// StringBuilder query = new StringBuilder(connectionNoListQuery);
		StringBuilder query = new StringBuilder(GARBAGE_SEARCH_CONNECTION_QUERY);
		// Add connection type
		addClauseIfRequired(preparedStatement, query);
		query.append(" wc.connectiontype = ? ");
		preparedStatement.add(connectionType);
