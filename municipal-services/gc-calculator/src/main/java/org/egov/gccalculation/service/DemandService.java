...
import org.egov.gccalculation.repository.DemandRepository;
...
import org.egov.gccalculation.repository.GCCalculationDaoImpl;
import org.egov.gccalculation.repository.GarbageConnectionRepository;
import org.egov.gccalculation.util.CalculatorUtil;
import org.egov.gccalculation.util.NotificationUtil;
import org.egov.gccalculation.util.GCCalculationUtil;
import org.egov.gccalculation.web.models.*;
import org.egov.gccalculation.web.models.Demand.StatusEnum;
...
import static org.egov.gccalculation.constants.GCCalculationConstant.ONE_TIME_FEE_SERVICE_FIELD;
import static org.egov.gccalculation.constants.GCCalculationConstant.MODIFY_WATER_CONNECTION;
...
public class DemandService {
	...
	@Autowired
	private GCCalculationUtil utils;

	@Autowired
	private GCCalculationConfiguration configs;
	...
	@Autowired
	private GarbageConnectionRepository waterConnectionRepository;
	...
	public List<Demand> generateDemand(CalculationReq request, List<Calculation> calculations,
		...
		Long fromDate = (Long) financialYearMaster.get(GCCalculationConstant.STARTING_DATE_APPLICABLES);
		Long toDate = (Long) financialYearMaster.get(GCCalculationConstant.ENDING_DATE_APPLICABLES);
	...
	private List<Demand> createDemand(RequestInfo requestInfo, List<Calculation> calculations,
		...
		for (Calculation calculation : calculations) {
			...
			GarbageConnectionRequest waterConnectionRequest = GarbageConnectionRequest.builder().waterConnection(connection)
		...
		//	log.info("Property: {}", property);
			...
			if (taxPeriodFrom == 0 && taxPeriodTo == 0) {
				taxPeriodFrom = (Long) financialYearMaster.get(GCCalculationConstant.STARTING_DATE_APPLICABLES);
				taxPeriodTo = (Long) financialYearMaster.get(GCCalculationConstant.ENDING_DATE_APPLICABLES);
			}
			Long expiryDaysInmillies = (Long) financialYearMaster.get(GCCalculationConstant.Demand_Expiry_Date_String);
	...
	private List<Demand> createDemand(CalculationReq calculationReq, List<Calculation> calculations,
		...
		for (Calculation calculation : calculations) {
			...
			Long expiryDate = (Long) financialYearMaster.get(GCCalculationConstant.Demand_Expiry_Date_String);
...
//	 					for (DemandDetail ddSew : demandDetails) {
//	 						DemandDetail dd1 = new DemandDetail();
//	 						if (ddSew.getTaxHeadMasterCode().equalsIgnoreCase(GCCalculationConstant.WS_CHARGE)) {
//	 							dd1.setTaxHeadMasterCode(GCCalculationConstant.SW_CHARGE);
//	 						} else if (ddSew.getTaxHeadMasterCode().equalsIgnoreCase(GCCalculationConstant.WS_Round_Off)) {
//	 							dd1.setTaxHeadMasterCode(GCCalculationConstant.SW_ROUND_OFF);
//	 						} else if (ddSew.getTaxHeadMasterCode().equalsIgnoreCase(GCCalculationConstant.WS_ADVANCE_CARRYFORWARD)) {
//	 							dd1.setTaxHeadMasterCode(GCCalculationConstant.SW_ADVANCE_CARRYFORWARD);
//	 						} else {
//	 							dd1.setTaxHeadMasterCode(GCCalculationConstant.SW_CHARGE);
//	 						}
//	 						dd1.setDemandId(ddSew.getDemandId());
...
//		log.info("waterConnectionRequest: {}", waterConnectionRequest);
	...
	private List<DemandDetail> getUpdatedDemandDetails(Calculation calculation, List<DemandDetail> demandDetails,
		...
		for (TaxHeadEstimate taxHeadEstimate : calculation.getTaxHeadEstimates()) {
			if (!taxHeadToDemandDetail.containsKey(taxHeadEstimate.getTaxHeadCode()) && !isDisconnectionRequest)
				...
				if (taxHeadEstimate.getTaxHeadCode().equalsIgnoreCase(GCCalculationConstant.WS_Round_Off))
					continue;
	...
	public void addRoundOffTaxHead(String tenantId, List<DemandDetail> demandDetails) {
		...
		for (DemandDetail demandDetail : demandDetails) {
			if (!demandDetail.getTaxHeadMasterCode().equalsIgnoreCase(GCCalculationConstant.WS_Round_Off))
		...
		if (roundOff.compareTo(BigDecimal.ZERO) != 0) {
			DemandDetail roundOffDemandDetail = DemandDetail.builder().taxAmount(roundOff)
					.taxHeadMasterCode(GCCalculationConstant.WS_Round_Off).tenantId(tenantId)
					.collectionAmount(BigDecimal.ZERO).build();
	...
	public List<Demand> updateDemands(GetBillCriteria getBillCriteria, RequestInfoWrapper requestInfoWrapper,
		...
		if (CollectionUtils.isEmpty(res.getDemands())) {
			...
			map.put(GCCalculationConstant.EMPTY_DEMAND_ERROR_CODE, GCCalculationConstant.EMPTY_DEMAND_ERROR_MESSAGE);
			throw new CustomException(map);
		}
		List<Demand> demands = res.getDemands();
		demands = demands.stream()
				.filter(i -> !GCCalculationConstant.DEMAND_CANCELLED_STATUS.equalsIgnoreCase(i.getStatus().toString()))
		...
		List<TaxPeriod> taxPeriods = mstrDataService.getTaxPeriodList(requestInfoWrapper.getRequestInfo(), tenantId,
				GCCalculationConstant.SERVICE_FIELD_VALUE_WS);
		consumerCodeToDemandMap.forEach((id, demand) -> {
			...
			log.info(" penalty taxhead code " + taxHeadMasterCodes.contains(GCCalculationConstant.WS_TIME_PENALTY));
			...
			if (!(isMigratedCon && oldDemand.getId().equalsIgnoreCase(demand.getId()))) {
				...
				if (!demand.getIsPaymentCompleted() && totalTax.compareTo(totalCollection) > 0
						&& !taxHeadMasterCodes.contains(GCCalculationConstant.WS_TIME_PENALTY)) {
					log.info(" Inside Update if ");

					if (demand.getStatus() != null && GCCalculationConstant.DEMAND_CANCELLED_STATUS
							.equalsIgnoreCase(demand.getStatus().toString()))
						throw new CustomException(GCCalculationConstant.EG_WS_INVALID_DEMAND_ERROR,
								GCCalculationConstant.EG_WS_INVALID_DEMAND_ERROR_MSG);
	...
	private List<Demand> updateDemandForCalculation(RequestInfo requestInfo, List<Calculation> calculations,
		...
		for (Calculation calculation : calculations) {
			...
			if (isForConnectionNo) {
				GarbageConnection connection = calculation.getWaterConnection();
				if (connection == null) {
					List<GarbageConnection> waterConnectionList = calculatorUtils.getWaterConnection(requestInfo,
	...
	private boolean applyTimeBasedApplicables(Demand demand, RequestInfoWrapper requestInfoWrapper,
		...
		if (expiryDate < System.currentTimeMillis()) {
			...
			for (DemandDetail detail : demand.getDemandDetails()) {
				if (GCCalculationConstant.TAX_APPLICABLE.contains(detail.getTaxHeadMasterCode())) {
					waterChargeApplicable = waterChargeApplicable.add(detail.getTaxAmount());
				}
				if (detail.getTaxHeadMasterCode().equalsIgnoreCase(GCCalculationConstant.WS_TIME_PENALTY)) {
					oldPenalty = oldPenalty.add(detail.getTaxAmount());
				}
				if (detail.getTaxHeadMasterCode().equalsIgnoreCase(GCCalculationConstant.WS_TIME_INTEREST)) {
					oldInterest = oldInterest.add(detail.getTaxAmount());
				}

				if (detail.getTaxHeadMasterCode().equalsIgnoreCase(GCCalculationConstant.WS_TIME_REBATE)) {
			...
			BigDecimal penalty = interestPenaltyRebateEstimates.get(GCCalculationConstant.WS_TIME_PENALTY);
			BigDecimal interest = interestPenaltyRebateEstimates.get(GCCalculationConstant.WS_TIME_INTEREST);
			BigDecimal rebate = interestPenaltyRebateEstimates.get(GCCalculationConstant.WS_TIME_REBATE);
			...
			if (interest.compareTo(BigDecimal.ZERO) != 0) {
				latestInterestDemandDetail = utils
						.getLatestDemandDetailByTaxHead(GCCalculationConstant.WS_TIME_INTEREST, details);
			...
			if (penalty.compareTo(BigDecimal.ZERO) != 0) {
				latestPenaltyDemandDetail = utils.getLatestDemandDetailByTaxHead(GCCalculationConstant.WS_TIME_PENALTY,
						details);
			...
			if (oldRebate.compareTo(BigDecimal.ZERO) != 0 || rebate.compareTo(BigDecimal.ZERO) != 0) {
				latestRebateDemandDetail = utils.getLatestDemandDetailByTaxHead(GCCalculationConstant.WS_TIME_REBATE,
						details);
			...
			if (!isPenaltyUpdated && penalty.compareTo(BigDecimal.ZERO) > 0)
				details.add(DemandDetail.builder().taxAmount(penalty.setScale(2, 2))
						.taxHeadMasterCode(GCCalculationConstant.WS_TIME_PENALTY).demandId(demandId).tenantId(tenantId)
						.build());
			if (!isInterestUpdated && interest.compareTo(BigDecimal.ZERO) > 0)
				details.add(DemandDetail.builder().taxAmount(interest.setScale(2, 2))
						.taxHeadMasterCode(GCCalculationConstant.WS_TIME_INTEREST).demandId(demandId).tenantId(tenantId)
						.build());

			if (!isRebateUpdated && rebate.compareTo(BigDecimal.ZERO) != 0)
				details.add(DemandDetail.builder().taxAmount(rebate.setScale(2, 2))
						.taxHeadMasterCode(GCCalculationConstant.WS_TIME_REBATE).demandId(demandId).tenantId(tenantId)
						.build());
		}
	...
	public String generateDemandForConsumerCode(RequestInfo requestInfo, BulkBillCriteria bulkBillCriteria) {
		...
		long startDay = (((int) billingMasterData.get(GCCalculationConstant.Demand_Generate_Date_String)) / 86400000);
		if (isCurrentDateIsMatching((String) billingMasterData.get(GCCalculationConstant.Billing_Cycle_String),
				startDay)) {
			...
			List<GarbageConnection> connections = waterCalculatorDao.getConnection(bulkBillCriteria.getTenantId(),
					bulkBillCriteria.getConsumerCode(), GCCalculationConstant.nonMeterdConnection, fromDate, toDate);
			...
			if (connections.size() > 0) {
				...
				for (GarbageConnection connection : connections) {
				...
				MigrationCount migrationCount = MigrationCount.builder().tenantid(bulkBillCriteria.getTenantId())
						.businessService("GC").limit((long) 1.00).id(UUID.randomUUID().toString()).offset((long) 1.00)
	...
	private boolean isValidBillingCycles(GarbageDetails waterConnection, RequestInfo requestInfo, String tenantId,
										 long taxPeriodFrom, long taxPeriodTo) {
	...
	public List<GarbageConnection> getConnectionPendingForDemand(RequestInfo requestInfo, String tenantId) {
		Map<String, Object> billingMasterData = calculatorUtils.loadBillingFrequencyMasterData(requestInfo, tenantId);
		List<GarbageConnection> connections = new ArrayList<GarbageConnection>();
		long startDay = (((int) billingMasterData.get(GCCalculationConstant.Demand_Generate_Date_String)) / 86400000);
		if (isCurrentDateIsMatching((String) billingMasterData.get(GCCalculationConstant.Billing_Cycle_String),
				startDay)) {
			...
			connections = waterCalculatorDao.getConnectionsNoListForDemand(tenantId,
					GCCalculationConstant.nonMeterdConnection, fromDate, toDate);
...
// ✅ Push leftover records if any
	...
	private boolean isValidBillingCycle(GarbageDetails waterConnection, RequestInfo requestInfo, String tenantId,
										long taxPeriodFrom, long taxPeriodTo) {
		// TODO Auto-generated method stub
	...
	private boolean fetchBill(GarbageDetails waterConnection, long taxPeriodFrom, long taxPeriodTo, String tenantId,
							  RequestInfo requestInfo) {
	...
	private boolean isCurrentDateIsMatching(String billingFrequency, long dayOfMonth) {
		if (billingFrequency.equalsIgnoreCase(GCCalculationConstant.Monthly_Billing_Period)
				&& (dayOfMonth <= LocalDateTime.now().getDayOfMonth())) {
			return true;
		} else if (billingFrequency.equalsIgnoreCase(GCCalculationConstant.Quaterly_Billing_Period)) {
			return false;
		}
		return true;
	}
	        ...
	        try {
	            ...
	            if (bills != null && !bills.isEmpty()) {
	            	  ...
	            	  billGeneratorDao.updateBillSchedulerConnectionStatus(
		            		 ...
		            		 schedlerId,
	  				        localitycode,
	  				        GCCalculationConstant.SUCCESS,
	  				        tenantId,
	  				        GCCalculationConstant.SUCCESS_MESSAGE,
	                ...
	                log.info("✅ Bill generated successfully for consumerCode: {}", consumerCode);
	            	  ...
	            	  billGeneratorDao.updateBillSchedulerConnectionStatus(
	            			  consumerCode,
			            		 schedlerId,
		  				        localitycode,
		  				        GCCalculationConstant.FAILURE,
		  				        tenantId,
		  				        GCCalculationConstant.FAILURE_MESSAGE,
		  				      System.currentTimeMillis()
		  				    );
	        	  ...
	        	  billGeneratorDao.updateBillSchedulerConnectionStatus(
	        			  consumerCode,
		            		 schedlerId,
	  				        ...
	  				        GCCalculationConstant.FAILURE,
	  				        tenantId,
	  				      GCCalculationConstant.FAILURE_MESSAGE + " | Error Msg: " + ex.getMessage(),
	  				      System.currentTimeMillis()
	  				    );
