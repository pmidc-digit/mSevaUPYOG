...
import org.egov.gccalculation.web.models.BillScheduler.StatusEnum;
...
import static org.egov.gccalculation.constants.GCCalculationConstant.*;
import static org.egov.gccalculation.web.models.TaxHeadCategory.CHARGES;

@Service
@Slf4j
public class GCCalculationServiceImpl implements GCCalculationService {
...
//			}
	...
	public Calculation getCalculation(RequestInfo requestInfo, CalculationCriteria criteria,
		...
		/*
		 * Long fromDate = (Long)
		 * financialYearMaster.get(GCCalculationConstant.STARTING_DATE_APPLICABLES);
		 * Long toDate = (Long)
		 * financialYearMaster.get(GCCalculationConstant.ENDING_DATE_APPLICABLES);
		 */
	...
	public void jobScheduler() {
		...
		for (String tenantId : tenantIds) {
			...
			String jsonPath = GCCalculationConstant.JSONPATH_ROOT_FOR_BilingPeriod;
	...
	public void getBillingPeriod(ArrayList<?> mdmsResponse, RequestInfo requestInfo, String tenantId) {
		...
		Long demandGenerateDateMillis = (Long) master.get(GCCalculationConstant.Demand_Generate_Date_String);
...
//		List<String> tenantIds = wSCalculationDao.getTenantId();
		...
		// Process each tenant - generate demands for both monthly and quarterly connections
		tenantIds.forEach(tenantId -> {
			try {
				...
				log.error("Exception occured while generating demand for tenant: " + tenantId, e);
			}
		});
	}
	...
	private void generateQuarterlyDemandsForTenant(String tenantId, RequestInfo requestInfo) {
		try {
			...
			masterDataService.enrichBillingPeriod(null, billingFrequencyMap, masterMap,
					GCCalculationConstant.nonMeterdConnection, "Quarterly");
...
//		List<String> tenantIds = wSCalculationDao.getTenantId();
	...
	public void generateBillBasedLocality(RequestInfo requestInfo) {
		...
		criteria.setStatus(GCCalculationConstant.INITIATED_CONST);
...
So, both lists are now filtered to include only records with INITIATED status, with an extra condition for Patiala that the group should not be present.  */		
	...
	private BigDecimal getWaterCessForDisconnection(Map<String, Object> masterMap, BigDecimal finalWaterCharge) {
		...
		timeBasedExemptionMasterMap.put(GCCalculationConstant.WC_WATER_CESS_MASTER,
				(JSONArray) (masterMap.getOrDefault(GCCalculationConstant.WC_WATER_CESS_MASTER, null)));

		if (timeBasedExemptionMasterMap.get(GCCalculationConstant.WC_WATER_CESS_MASTER) != null) {
			List<Object> waterCessMasterList = timeBasedExemptionMasterMap
					.get(GCCalculationConstant.WC_WATER_CESS_MASTER);

			Map<String, Object> CessMap = mDataService.getApplicableMasterCess(GCCalculationConstant.Assessment_Year,
					waterCessMasterList);
