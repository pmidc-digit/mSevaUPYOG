...
import org.egov.gccalculation.web.models.TaxHeadEstimate;
...
public class PayService {
	...
	public TaxHeadEstimate roundOfDecimals(BigDecimal creditAmount, BigDecimal debitAmount, boolean isConnectionFee) {
        ...
        String taxHead = isConnectionFee ? GCCalculationConstant.WS_Round_Off : GCCalculationConstant.WS_ONE_TIME_FEE_ROUND_OFF;
	...
	public Map<String, BigDecimal> applyPenaltyRebateAndInterest(BigDecimal waterCharge,
		...
		BigDecimal penalty = getApplicablePenalty(demand,waterCharge, noOfDays, timeBasedExemptionMasterMap.get(GCCalculationConstant.WC_PENANLTY_MASTER));
		BigDecimal interest = getApplicableInterest(demand,waterCharge, noOfDays, timeBasedExemptionMasterMap.get(GCCalculationConstant.WC_INTEREST_MASTER));
		//TODO: Disabling REBATE for now
//		BigDecimal rebate = getApplicableRebate(waterCharge, demand, timeBasedExemptionMasterMap.get(GCCalculationConstant.WC_REBATE_MASTER));

		log.info("time based applicables are: penalty = " + penalty +" interest = "+  interest
//				+ " rebate = " +rebate
				);
		estimates.put(GCCalculationConstant.WS_TIME_PENALTY, penalty.setScale(2, 2));
		estimates.put(GCCalculationConstant.WS_TIME_INTEREST, interest.setScale(2, 2));
//		estimates.put(GCCalculationConstant.WS_TIME_REBATE, rebate.negate().setScale(2, 2));
		return estimates;
	}
	...
	public BigDecimal getApplicablePenalty(Demand demand,BigDecimal waterCharge, BigDecimal noOfDays, JSONArray config) {
		...
		BigDecimal daysApplicable = null != penaltyMaster.get(GCCalculationConstant.DAYA_APPLICABLE_NAME)
				? BigDecimal.valueOf(((Number) penaltyMaster.get(GCCalculationConstant.DAYA_APPLICABLE_NAME)).intValue())
				: null;
		...
		BigDecimal rate = null != penaltyMaster.get(GCCalculationConstant.RATE_FIELD_NAME)
				? BigDecimal.valueOf(((Number) penaltyMaster.get(GCCalculationConstant.RATE_FIELD_NAME)).doubleValue())
				: null;

		BigDecimal flatAmt = null != penaltyMaster.get(GCCalculationConstant.FLAT_AMOUNT_FIELD_NAME)
				? BigDecimal
						.valueOf(((Number) penaltyMaster.get(GCCalculationConstant.FLAT_AMOUNT_FIELD_NAME)).doubleValue())
				: BigDecimal.ZERO;
		...
		if (rate == null)
			...
			// rate of penalty
			applicablePenalty = waterCharge.multiply(rate.divide(GCCalculationConstant.HUNDRED));
		}
		return applicablePenalty;
	}
	...
	public BigDecimal getApplicableInterest(Demand demand,BigDecimal waterCharge, BigDecimal noOfDays, JSONArray config) {
		...
		BigDecimal daysApplicable = null != interestMaster.get(GCCalculationConstant.DAYA_APPLICABLE_NAME)
				? BigDecimal.valueOf(((Number) interestMaster.get(GCCalculationConstant.DAYA_APPLICABLE_NAME)).intValue())
				: null;
		if (daysApplicable == null)
...
//				.subtract(BigDecimal.valueOf(Double.parseDouble(interestMaster.get(GCCalculationConstant.STARTING_DATE_APPLICABLES).toString())));
//		log.info("No. of days applicable for interest ::" + noOfDaysforInterest );
		
		BigDecimal rate = null != interestMaster.get(GCCalculationConstant.RATE_FIELD_NAME)
				? BigDecimal.valueOf(((Number) interestMaster.get(GCCalculationConstant.RATE_FIELD_NAME)).doubleValue())
				: null;

		BigDecimal flatAmt = null != interestMaster.get(GCCalculationConstant.FLAT_AMOUNT_FIELD_NAME)
				? BigDecimal
						.valueOf(((Number) interestMaster.get(GCCalculationConstant.FLAT_AMOUNT_FIELD_NAME)).doubleValue())
				: BigDecimal.ZERO;
		...
		if (rate == null)
			...
			// rate of interest
			applicableInterest = waterCharge.multiply(rate.divide(GCCalculationConstant.HUNDRED));
		}
	...
	public BigDecimal getApplicableRebate(BigDecimal waterCharge, Demand demand, JSONArray config) {
		...
		BigDecimal daysApplicable = null != rebateMaster.get(GCCalculationConstant.ENDING_DATE_APPLICABLES)
				? BigDecimal.valueOf(((Number) rebateMaster.get(GCCalculationConstant.ENDING_DATE_APPLICABLES)).intValue())
				: null;
		...
		BigDecimal rate = null != rebateMaster.get(GCCalculationConstant.RATE_FIELD_NAME)
				? BigDecimal.valueOf(((Number) rebateMaster.get(GCCalculationConstant.RATE_FIELD_NAME)).doubleValue())
				: null;

		BigDecimal flatAmt = null != rebateMaster.get(GCCalculationConstant.FLAT_AMOUNT_FIELD_NAME)
				? BigDecimal
						.valueOf(((Number) rebateMaster.get(GCCalculationConstant.FLAT_AMOUNT_FIELD_NAME)).doubleValue())
				: BigDecimal.ZERO;
		...
		if (rate == null)
			...
			// rate of interest
			applicableRebate = waterCharge.multiply(rate.divide(GCCalculationConstant.HUNDRED));
		}
