...
import org.egov.gccalculation.web.models.users.User;
...
public class PaymentNotificationService {
		...
		try {
			...
			if (configuredChannelNames.contains(CHANNEL_NAME_EVENT)) {
				if (config.getIsUserEventsNotificationEnabled() != null && config.getIsUserEventsNotificationEnabled()) {
					if (mappedRecord.get(serviceName).equalsIgnoreCase(GCCalculationConstant.SERVICE_FIELD_VALUE_WS)) {
	...
	private EventRequest getEventRequest(HashMap<String, String> mappedRecord, GarbageConnectionRequest waterConnectionRequest, String topic,
		...
		for (String mobile : mobileNumbers) {
			...
			events.add(Event.builder().tenantId(property.getTenantId())
					...
					.eventType(GCCalculationConstant.USREVENTS_EVENT_TYPE)
					.name(GCCalculationConstant.USREVENTS_EVENT_NAME)
					.postedBy(GCCalculationConstant.USREVENTS_EVENT_POSTEDBY).source(Source.WEBAPP).recepient(recepient)
	...
	public Map<String, String> getMessageForMobileNumber(Map<String, String> mobileNumbersAndNames,
		...
		for (Entry<String, String> mobileAndName : mobileNumbersAndNames.entrySet()) {
			...
			if (messageToReplace.contains("{Service}"))
				messageToReplace = messageToReplace.replace("{Service}", GCCalculationConstant.SERVICE_FIELD_VALUE_WS);
	...
	public String getBillNotificationMessage(String message,Map<String, Object> masterMap,User user) {
		...
		if (message.contains("{Service}"))
			message = message.replace("{Service}", GCCalculationConstant.SERVICE_FIELD_VALUE_WS);
...
log.info("Sending Bill Notification");
		...
		if (configuredChannelNames.contains(CHANNEL_NAME_SMS)) {
			if (config.getIsSMSEnabled() != null && config.getIsSMSEnabled()) {
				...
				if (messageString == null) {
					log.info("EG_WS No message Found For " + GCCalculationConstant.BILL_FAILURE_MESSAGE_SMS+" or "+BILL_SUCCESS_MESSAGE_SMS);
					return;
				}
		...
		if (configuredChannelNames.contains(CHANNEL_NAME_EMAIL)) {
			if (config.getIsEmailEnabled() != null && config.getIsEmailEnabled()) {
				...
				if (messageString == null) {
					log.info("EG_WS No message Found For " + GCCalculationConstant.BILL_SUCCESS_MESSAGE_EMAIL+" or "+BILL_FAILURE_MESSAGE_EMAIL );
					return;
				}
