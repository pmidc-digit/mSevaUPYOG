...
public class DiffService {
	 ...
	 */
	public void checkDifferenceAndSendEditNotification(GarbageConnectionRequest request) {
		try {
	 ...
	 * 
	 * @param updateConnection GarbageConnection Object
	 * @param searchResult GarbageConnection Object
	 * @return List of updated fields
	 */
	private List<String> getUpdateFields(GarbageConnection updateConnection, GarbageConnection searchResult) {
		...
		changes.forEach(change -> {
			if (GCConstants.FIELDS_TO_CHECK.contains(change.getPropertyName())) {
	...
	private List<String> getObjectsAdded(GarbageConnection updateConnection, GarbageConnection searchResult) {
		Javers javers = JaversBuilder.javers().build();
		Diff diff = javers.compare(updateConnection, searchResult);
		...
		for(Object object: objectsAdded) {
			...
			if (!classModified.contains(className) && !GCConstants.IGNORE_CLASS_ADDED.contains(className))
	 ...
	 */
    private List<String> getObjectsRemoved(GarbageConnection updateConnection, GarbageConnection searchResult) {
