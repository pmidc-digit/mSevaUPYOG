...
public class EODBredirect {
    ...
    public boolean runEodbFlow(GarbageConnectionRequest connection) {
    ...
    private boolean updateStatusWithToken(String token, GarbageConnectionRequest connection) {
        String apiUrl = updateUrl;

        // Extract additionalDetails from the connection (assuming it is a Map)
        Object additionalDetailsObject = connection.getGarbageConnection().getAdditionalDetails();  // This should be a Map<String, Object>
        ...
        String applicationStatus = connection.getGarbageConnection().getApplicationStatus();
