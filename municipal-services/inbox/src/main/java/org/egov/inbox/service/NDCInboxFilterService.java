...
//            }
            ...
            if(!ObjectUtils.isEmpty(processCriteria.getStatus())){
            	if(processCriteria.getModuleName().equals(NDC)) { //if needed for all modules, replicate
            ...
            searchCriteria.put(LIMIT_PARAM, criteria.getLimit());
...
//            acknowledgementNumbers = JsonPath.read(result, "$.Licenses.*.applicationnumber"); 
          acknowledgementNumbers = JsonPath.read(result, "$.Services.*.servicerequestid");
          //  acknowledgementNumbers = JsonPath.read(result, "$.Applications.*.uuid");
    ...
    public Integer fetchApplicationCountFromSearcher(InboxSearchCriteria criteria, HashMap<String, String> StatusIdNameMap, RequestInfo requestInfo){
        ...
        if(!isSearchResultEmpty){
            ...
            if(moduleSearchCriteria.containsKey(NDC_lOCALITY_PARAM)){
                searchCriteria.put(NDC_lOCALITY_PARAM, moduleSearchCriteria.get(NDC_lOCALITY_PARAM));
            }
            if(moduleSearchCriteria.containsKey(NDC_APPLICATION_NUMBER_PARAM)){
            ...
            StringBuilder uri = new StringBuilder();
            uri.append(searcherHost).append(ndcInboxSearcherCountEndpoint);
