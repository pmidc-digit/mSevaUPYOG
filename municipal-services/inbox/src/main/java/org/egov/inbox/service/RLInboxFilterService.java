...
@Slf4j
@Service
public class RLInboxFilterService {
    ...
    @Value("${egov.searcher.rl.search.path:}")
    private String rlInboxSearcherEndpoint;

    @Value("${egov.searcher.rl.search.desc.path:}")
    private String rlInboxSearcherDescEndpoint;

    @Value("${egov.searcher.rl.count.path:}")
    private String rlInboxSearcherCountEndpoint;
    ...
    public List<String> fetchApplicationNumbersFromSearcher(InboxSearchCriteria criteria,
        ...
        // Fetch application numbers from RL allotment searcher response
        List<String> applicationNumbers = JsonPath.read(result, "$.allotmentApplication.[*].application_number");
    ...
    private String resolveSearchEndpoint(HashMap<String, Object> moduleSearchCriteria) {
        ...
        if (moduleSearchCriteria.containsKey(SORT_ORDER_PARAM)
                ...
                && rlInboxSearcherDescEndpoint != null && !rlInboxSearcherDescEndpoint.isEmpty()) {

            uri.append(searcherHost).append(rlInboxSearcherDescEndpoint);

        } else if (rlInboxSearcherEndpoint != null && !rlInboxSearcherEndpoint.isEmpty()) {
            uri.append(searcherHost).append(rlInboxSearcherEndpoint);
    ...
    private Map<String, Object> getSearchCriteria(InboxSearchCriteria criteria,
        ...
        if (moduleSearchCriteria != null && moduleSearchCriteria.containsKey("allotmentStatus")) {
            searchCriteria.put("allotmentStatus", moduleSearchCriteria.get("allotmentStatus"));
        }

        if (moduleSearchCriteria != null && moduleSearchCriteria.containsKey("expireFlag")) {
            searchCriteria.put("expireFlag", moduleSearchCriteria.get("expireFlag"));
        }
        ...
        // Accommodating processSearchCriteria in the search_criteria
        if(!ObjectUtils.isEmpty(processCriteria.getStatus())){
            List<String> matchingIdsRl = statusIdNameMap.entrySet().stream()
                    .filter(entry -> processCriteria.getStatus().contains(entry.getValue()))
                    .map(Map.Entry::getKey)
                    .collect(Collectors.toList());
            searchCriteria.put(STATUS_PARAM, matchingIdsRl);
        } else {
    ...
    public Integer fetchApplicationCountFromSearcher(InboxSearchCriteria criteria,
        ...
        if (rlInboxSearcherCountEndpoint != null && !rlInboxSearcherCountEndpoint.isEmpty()) {
            HashMap<String, Object> moduleSearchCriteria = criteria.getModuleSearchCriteria();
            ProcessInstanceSearchCriteria processCriteria = criteria.getProcessSearchCriteria();
            Boolean isSearchResultEmpty = false;
            Boolean isMobileNumberPresent = false;
            ...
            citizenUri.append(searcherHost).append(rlInboxSearcherCountEndpoint);
    ...
    /**
     * Fetch user UUID based on mobile number
     */
