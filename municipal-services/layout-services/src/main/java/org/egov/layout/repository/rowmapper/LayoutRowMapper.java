...
@Component
public class LayoutRowMapper implements ResultSetExtractor<List<Layout>> {
	/**
	...
	public List<Layout> extractData(ResultSet rs) throws SQLException, DataAccessException {
		...
		while (rs.next()) {
			...
			if (nocListMap.getOrDefault(Id, null) == null) {
                ...
                noc.setLayoutNo(rs.getString("layoutNo"));
                noc.setLayoutType(rs.getString("layoutType"));
...
//                layout.getNocDetails().setAdditionalDetails(additionalDetails);
		...
		Map<String, Layout> sortedMap = nocListMap.entrySet()
				.stream()
				.sorted((e1, e2) -> {
...
//	private void addChildrenToProperty(ResultSet rs, Layout layout) throws SQLException {
...
//			LayoutDetails nocdetails = new LayoutDetails();
	...
	private void addChildrenToProperty(ResultSet rs, Layout noc) throws SQLException {
		...
		if (!StringUtils.isEmpty(documentsJson)) {
			...
			if (!StringUtils.isEmpty(nocDetailsJson) && !"null".equals(nocDetailsJson)) {
				try {
					LayoutDetails details = new Gson().fromJson(nocDetailsJson, LayoutDetails.class);
					details.setAuditDetails(noc.getAuditDetails());
					details.setLayoutId(noc.getId());
