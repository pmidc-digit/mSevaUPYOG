 ...
 */
package org.egov.layout.web.controller;
...
import org.egov.layout.web.model.LayoutResponse;
...
public class LAYOUTController {
	...
	public ResponseEntity<LayoutResponse> create(@Valid @RequestBody LayoutRequest nocRequest) {
		List<Layout> nocList = nocService.create(nocRequest);
		LayoutResponse response = LayoutResponse.builder().noc(nocList)
	...
	public ResponseEntity<LayoutResponse> update(@Valid @RequestBody LayoutRequest nocRequest) {
		List<Layout> nocList = nocService.update(nocRequest);
		LayoutResponse response = LayoutResponse.builder().noc(nocList)
				.responseInfo(responseInfoFactory.createResponseInfoFromRequestInfo(nocRequest.getRequestInfo(), true))
				.build();
		return new ResponseEntity<>(response, HttpStatus.OK);
	...
	public ResponseEntity<LayoutResponse> search(@Valid @RequestBody RequestInfoWrapper requestInfoWrapper,
												 @Valid @ModelAttribute LayoutSearchCriteria criteria) {

		List<Layout> nocList = nocService.search(criteria, requestInfoWrapper.getRequestInfo());
