...
public class PropertyQueryBuilder {
	...
	// property.additionaldetails as padditionaldetails, (CASE WHEN
	// property.status='ACTIVE' then 0 WHEN property.status='INWORKFLOW' then 1 WHEN
	// property.status='INACTIVE' then 2 ELSE 3 END) as statusorder, ";
...
//				builder.append(AND_QUERY);
	...
	public String getPropertySearchQuerytemp(Property criteria, List<Object> preparedStmtList,
		...
		if (!propertyIds.isEmpty()) {
			...
			preparedStmtList.add(propertyIds);
		}
		...
		if (!acknowledgementIds.isEmpty()) {
			...
			addClauseIfRequired(preparedStmtList, builder);
			builder.append("property.acknowldgementnumber IN (?)");
			preparedStmtList.add(acknowledgementIds);
		}
