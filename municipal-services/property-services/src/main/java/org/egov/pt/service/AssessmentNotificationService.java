...
public class AssessmentNotificationService {
	...
	@Autowired
	public AssessmentNotificationService(NotificationUtil util, PropertyService propertyService,
			PropertyConfiguration config, BillingService billingService, UnmaskingUtil unmaskingUtil) {
		this.util = util;
	...
	public void process(String topicName, AssessmentRequest assessmentRequest) {
		...
		PropertyCriteria criteria = PropertyCriteria.builder().tenantId(tenantId)
				.propertyIds(Collections.singleton(assessment.getPropertyId())).isSearchInternal(Boolean.TRUE).build();
		...
		try {
			...
			if (billResponse != null) {
				...
				List<String> configuredChannelNamesForAssessment = util.fetchChannelList(new RequestInfo(), tenantId,
						PT_BUSINESSSERVICE, ACTION_FOR_ASSESSMENT);
	...
	private void enrichSMSRequestForDues(List<SMSRequest> smsRequests, AssessmentRequest assessmentRequest,
		...
		if (messageTemplate.contains(NOTIFICATION_FINANCIALYEAR))
			messageTemplate = messageTemplate.replace(NOTIFICATION_FINANCIALYEAR,
					assessmentRequest.getAssessment().getFinancialYear());
	 ...
	 * @return
	 */
	private String getCustomizedMsg(String topicName, AssessmentRequest request, Property property,
		...
		if (processInstance == null) {
			...
			if (topicName.equalsIgnoreCase(config.getCreateAssessmentTopic()))
				...
				msgCode = NOTIFICATION_ASSESSMENT_UPDATE;
