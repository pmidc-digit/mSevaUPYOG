...
@Service
public class ClosureWorkflowService {

	@Autowired
	private RentLeaseConfiguration configs;

	@Autowired
	private org.egov.rl.repository.ServiceRequestRepository restRepo;
	...
	public void updateWorkflowStatus(ClsureRequest closureRequest) {
		AllotmentClsure allotmentClsure = closureRequest.getAllotmentClsure();
			ProcessInstance processInstance = getProcessInstanceForClosure(allotmentClsure,
					closureRequest.getRequestInfo());
			ProcessInstanceRequest workflowRequest = new ProcessInstanceRequest(closureRequest.getRequestInfo(),
					Collections.singletonList(processInstance));
			State state = callWorkFlow(workflowRequest);
			allotmentClsure.setStatus(state.getApplicationStatus());			
	}
	...
	private ProcessInstance getProcessInstanceForClosure(AllotmentClsure allotmentClsure, RequestInfo requestInfo) {
		...
		processInstance.setBusinessService("CLOSE_RENT_N_LEASE");
